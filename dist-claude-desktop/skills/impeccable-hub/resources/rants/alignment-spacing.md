# Rant: Alignment and spacing — the "floating awkwardly in a random spot" failure
...
This is the signature discipline showing up as a refusal. The rant has two specific failures layered:
...
Naming a related pattern that violates both the alignment register AND the typography register (specifically `rants/typography-mono.md`): a segmented control or button where every option carries a mono "tag chip" prefixing the human label that adds no information.
...
See `~/.claude/docs/concepts/design-contract/rants/typography-mono.md` for the full mono-discipline coverage of this pattern.
