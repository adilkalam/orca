...
function estimateTokens(entry: StoredEntry<unknown>): { tokens: number; source: 'reported' | 'estimated' } {
...
function renderThought(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.branchId) lines.push(`**Branch**: ${c.branchId}`);
  if (c.isRevision) lines.push(`**Revises**: Thought ${c.revisesThought}`);
  if (c.thoughtNumber || c.totalThoughts) {
    lines.push(`**Progress**: ${c.thoughtNumber || '?'}/${c.totalThoughts || '?'}`);
  }
  lines.push('');
  if (c.thought) lines.push(String(c.thought));
  return lines.join('\n');
}

function renderMentalModel(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.modelName) lines.push(`**Model**: ${c.modelName}`);
  if (c.problem) lines.push(`**Problem**: ${c.problem}`);
  if (c.setup) lines.push(`\n**Setup**: ${c.setup}`);
  ...
  if (Array.isArray(c.rootCauses) && c.rootCauses.length > 0) {
    lines.push('\n**Root Causes**:');
    for (const rc of c.rootCauses as Array<{ failure: string; cause: string; preventable: boolean }>) {
...
function renderCheckpointContent(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.summary) lines.push(String(c.summary));
  if (Array.isArray(c.keyFindings) && c.keyFindings.length > 0) {
    lines.push('\n**Key Findings**:');
    for (const f of c.keyFindings) lines.push(`- ${f}`);
  }
  ...
  if (Array.isArray(c.nextSteps) && c.nextSteps.length > 0) {
    lines.push('\n**Next Steps**:');
    for (const s of c.nextSteps) lines.push(`- ${s}`);
  }
  return lines.join('\n');
}

function renderDecision(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.statement) lines.push(`**Decision**: ${c.statement}`);
  if (Array.isArray(c.options)) {
    lines.push('\n**Options**:');
    for (const opt of c.options as Array<{ name: string; description: string; pros?: string[]; cons?: string[] }>) {
  ...
  if (Array.isArray(c.criteria) && c.criteria.length > 0) {
    lines.push(`\n**Criteria**: ${(c.criteria as string[]).join(', ')}`);
  }
  if (c.weights) lines.push(`**Weights**: ${JSON.stringify(c.weights)}`);
  if (c.scores) lines.push(`**Scores**: ${JSON.stringify(c.scores)}`);
  if (c.analysis) lines.push(`\n**Analysis**: ${c.analysis}`);
  if (c.choice) lines.push(`\n**Choice**: ${c.choice}`);
  if (c.confidence !== undefined) lines.push(`**Confidence**: ${c.confidence}`);
  return lines.join('\n');
}

function renderMeta(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.process) lines.push(`**Process**: ${c.process}`);
  ...
  if (Array.isArray(c.adjustments) && c.adjustments.length > 0) {
    lines.push('\n**Adjustments**:');
    for (const a of c.adjustments) lines.push(`- ${a}`);
  }
  if (c.insights) lines.push(`\n**Insights**: ${c.insights}`);
  if (c.effectiveness !== undefined) lines.push(`**Effectiveness**: ${c.effectiveness}`);
  if (c.defaultCounterfactual) {
    const dc = c.defaultCounterfactual as { trainedDefault: string; reasonedConclusion: string; gap: string };
  ...
  if (Array.isArray(c.reflexesObserved) && c.reflexesObserved.length > 0) {
    lines.push('\n**Reflexes Observed**:');
    for (const r of c.reflexesObserved as Array<{ reflex: string; description: string; caught: boolean }>) {
      lines.push(`- ${r.reflex}: ${r.description} (caught: ${r.caught})`);
    }
  }
  if (c.arcPosition) lines.push(`**Arc Position**: ${c.arcPosition}`);
  return lines.join('\n');
}

function renderSystems(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.system) lines.push(`**System**: ${c.system}`);
  if (Array.isArray(c.components) && c.components.length > 0) {
    lines.push('\n**Components**:');
    for (const comp of c.components as Array<{ name: string; function: string }>) {
      lines.push(`- **${comp.name}**: ${comp.function}`);
    }
  }
  if (Array.isArray(c.relationships) && c.relationships.length > 0) {
    lines.push('\n**Relationships**:');
    for (const rel of c.relationships as Array<{ from: string; to: string; type: string }>) {
      lines.push(`- ${rel.from} -> ${rel.to} (${rel.type})`);
    }
  }
  if (Array.isArray(c.feedbackLoops) && c.feedbackLoops.length > 0) {
    lines.push('\n**Feedback Loops**:');
    for (const loop of c.feedbackLoops) lines.push(`- ${loop}`);
  }
  return lines.join('\n');
}

function renderCollaborative(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.topic) lines.push(`**Topic**: ${c.topic}`);
  if (Array.isArray(c.perspectives)) {
    lines.push('\n**Perspectives**:');
    for (const p of c.perspectives as Array<{ role: string; viewpoint: string; arguments: string[] }>) {
      lines.push(`\n  **${p.role}**: ${p.viewpoint}`);
      if (p.arguments?.length) {
        for (const a of p.arguments) lines.push(`  - ${a}`);
      }
    }
  }
  if (Array.isArray(c.commonGround) && c.commonGround.length > 0) {
    lines.push('\n**Common Ground**:');
    for (const cg of c.commonGround) lines.push(`- ${cg}`);
  }
  if (Array.isArray(c.tensions) && c.tensions.length > 0) {
    lines.push('\n**Tensions**:');
    for (const t of c.tensions) lines.push(`- ${t}`);
  }
  if (c.synthesis) lines.push(`\n**Synthesis**: ${c.synthesis}`);
  return lines.join('\n');
}

function renderCreative(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.prompt) lines.push(`**Prompt**: ${c.prompt}`);
  if (Array.isArray(c.techniques) && c.techniques.length > 0) {
    lines.push(`**Techniques**: ${(c.techniques as string[]).join(', ')}`);
  }
  if (Array.isArray(c.ideas) && c.ideas.length > 0) {
    lines.push('\n**Ideas**:');
    for (const idea of c.ideas as Array<{ idea: string; potential: string; challenges: string[] }>) {
      lines.push(`\n  - **${idea.idea}**`);
      lines.push(`    Potential: ${idea.potential}`);
      if (idea.challenges?.length) lines.push(`    Challenges: ${idea.challenges.join(', ')}`);
    }
  }
  if (c.synthesis) lines.push(`\n**Synthesis**: ${c.synthesis}`);
  return lines.join('\n');
...
function renderArgumentation(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.claim) lines.push(`**Claim**: ${c.claim}`);
  if (Array.isArray(c.premises) && c.premises.length > 0) {
    lines.push('\n**Premises**:');
    for (const p of c.premises) lines.push(`- ${p}`);
  }
  if (Array.isArray(c.evidence) && c.evidence.length > 0) {
    lines.push('\n**Evidence**:');
    for (const e of c.evidence as Array<{ point: string; strength: string; source?: string }>) {
      lines.push(`- ${e.point} [${e.strength}]${e.source ? ` (${e.source})` : ''}`);
    }
  }
  if (Array.isArray(c.counterarguments) && c.counterarguments.length > 0) {
    lines.push('\n**Counterarguments**:');
    for (const ca of c.counterarguments as Array<{ point: string; rebuttal: string }>) {
      lines.push(`- ${ca.point}`);
...
function renderTree(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.root) lines.push(`**Root**: ${c.root}`);
  if (Array.isArray(c.branches) && c.branches.length > 0) {
    lines.push('\n**Branches**:');
    for (const b of c.branches as Array<{ id: string; thought: string; evaluation?: unknown; score?: number }>) {
  ...
  if (Array.isArray(c.bestPath) && c.bestPath.length > 0) {
    lines.push(`\n**Best Path**: ${(c.bestPath as string[]).join(' -> ')}`);
  }
  if (Array.isArray(c.pruned) && c.pruned.length > 0) {
    lines.push(`**Pruned**: ${(c.pruned as string[]).join(', ')}`);
  }
  if (c.synthesis) lines.push(`\n**Synthesis**: ${c.synthesis}`);
  return lines.join('\n');
}

function renderCausal(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.phenomenon) lines.push(`**Phenomenon**: ${c.phenomenon}`);
  if (Array.isArray(c.causes) && c.causes.length > 0) {
    lines.push('\n**Causes**:');
    for (const cause of c.causes as Array<{ factor: string; type: string; strength: string; evidence?: string }>) {
  ...
  if (Array.isArray(c.effects) && c.effects.length > 0) {
    lines.push('\n**Effects**:');
    for (const e of c.effects as Array<{ outcome: string; likelihood: string; timeframe: string }>) {
      lines.push(`- ${e.outcome} (${e.likelihood}, ${e.timeframe})`);
    }
  }
  if (Array.isArray(c.chains) && c.chains.length > 0) {
    lines.push('\n**Causal Chains**:');
    for (const chain of c.chains as Array<{ sequence: string[]; probability: number }>) {
      lines.push(`- ${chain.sequence.join(' -> ')} (p=${chain.probability})`);
    }
  }
  if (Array.isArray(c.interventions) && c.interventions.length > 0) {
    lines.push('\n**Interventions**:');
    for (const i of c.interventions) lines.push(`- ${i}`);
  }
  return lines.join('\n');
}

function renderSimulation(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.scenario) lines.push(`**Scenario**: ${c.scenario}`);
  if (Array.isArray(c.initialConditions) && c.initialConditions.length > 0) {
    lines.push('\n**Initial Conditions**:');
    for (const ic of c.initialConditions as Array<{ variable: string; value: string }>) {
      lines.push(`- ${ic.variable}: ${ic.value}`);
    }
  }
  if (Array.isArray(c.steps) && c.steps.length > 0) {
    lines.push('\n**Steps**:');
    for (const s of c.steps as Array<{ step: number; action: string; outcome: string }>) {
      lines.push(`${s.step}. ${s.action} -> ${s.outcome}`);
    }
  }
  if (c.finalState) lines.push(`\n**Final State**: ${c.finalState}`);
  if (Array.isArray(c.insights) && c.insights.length > 0) {
    lines.push('\n**Insights**:');
    for (const i of c.insights) lines.push(`- ${i}`);
  }
  if (Array.isArray(c.alternativeOutcomes) && c.alternativeOutcomes.length > 0) {
    lines.push('\n**Alternative Outcomes**:');
    for (const ao of c.alternativeOutcomes) lines.push(`- ${ao}`);
  }
  return lines.join('\n');
}

function renderUlysses(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.goal) lines.push(`**Goal**: ${c.goal}`);
  if (Array.isArray(c.temptations) && c.temptations.length > 0) {
    lines.push('\n**Temptations**:');
    for (const t of c.temptations as Array<{ trigger: string; temptation: string; risk: string }>) {
      lines.push(`- ${t.temptation} (trigger: ${t.trigger}, risk: ${t.risk})`);
    }
  }
  if (Array.isArray(c.commitments) && c.commitments.length > 0) {
    lines.push('\n**Commitments**:');
    for (const cm of c.commitments as Array<{ commitment: string; enforcement: string; consequences: string }>) {
      lines.push(`- ${cm.commitment} (enforcement: ${cm.enforcement})`);
    }
  }
  if (Array.isArray(c.safeguards) && c.safeguards.length > 0) {
    lines.push('\n**Safeguards**:');
    for (const sg of c.safeguards as Array<{ safeguard: string; trigger: string }>) {
      lines.push(`- ${sg.safeguard} (trigger: ${sg.trigger})`);
    }
  }
  if (c.escapeHatch) lines.push(`\n**Escape Hatch**: ${c.escapeHatch}`);
  return lines.join('\n');
}

function renderOODA(c: Record<string, unknown>): string {
  const lines: string[] = [];
  if (c.situation) lines.push(`**Situation**: ${c.situation}`);
  if (c.iteration !== undefined) lines.push(`**Iteration**: ${c.iteration}`);
  if (c.observe) {
    const obs = c.observe as { data?: string[]; environment?: string; changes?: string[] };
    lines.push('\n**Observe**:');
    if (obs.environment) lines.push(`  Environment: ${obs.environment}`);
    if (obs.data?.length) {
      for (const d of obs.data) lines.push(`  - ${d}`);
    }
    if (obs.changes?.length) {
      lines.push('  Changes:');
      for (const ch of obs.changes) lines.push(`  - ${ch}`);
    }
  }
  if (c.orient) {
    const ori = c.orient as { analysis?: string; mentalModels?: string[] };
    lines.push('\n**Orient**:');
    if (ori.analysis) lines.push(`  ${ori.analysis}`);
    if (ori.mentalModels?.length) lines.push(`  Models: ${ori.mentalModels.join(', ')}`);
  }
  if (c.decide) {
    const dec = c.decide as { selectedOption?: string; reasoning?: string };
    lines.push('\n**Decide**:');
    if (dec.selectedOption) lines.push(`  Selected: ${dec.selectedOption}`);
    if (dec.reasoning) lines.push(`  Reasoning: ${dec.reasoning}`);
  }
  if (c.act) {
    const act = c.act as { action?: string; implementation?: string[]; feedback?: string };
    lines.push('\n**Act**:');
    if (act.action) lines.push(`  Action: ${act.action}`);
    if (act.implementation?.length) {
      for (const impl of act.implementation) lines.push(`  - ${impl}`);
    }
    if (act.feedback) lines.push(`  Feedback: ${act.feedback}`);
  }
  return lines.join('\n');
}

function renderOrchestration(c: Record<string, unknown>): string {
  const lines: string[] = [];
  ...
  if (Array.isArray(c.suggestedOperations) && c.suggestedOperations.length > 0) {
    ...
    for (const op of c.suggestedOperations as Array<{ order: number; operation: string; reason: string }>) {
...
function renderGeneric(c: Record<string, unknown>): string {
  const lines: string[] = [];
  const textValue = c.text;
  for (const [key, value] of Object.entries(c)) {
    if (key === 'text' || key === 'nextThoughtNeeded') continue;
...
const RENDERERS: Record<string, (c: Record<string, unknown>) => string> = {
...
function renderEntryContent(storeKey: string, content: Record<string, unknown>): string {
...
function renderCheckpointHeader(c: Record<string, unknown>): string {
  const parts: string[] = [];
  if (c.phase) parts.push(`**Phase**: ${c.phase}`);
  if (c.gateCheck) {
    const gc = c.gateCheck as { selfCheckPassed: boolean; depthGatePassed: boolean; notes?: string };
  ...
  if (Array.isArray(c.resolveConstraints) && c.resolveConstraints.length > 0) {
    parts.push(`**Resolved**: ${(c.resolveConstraints as string[]).join(', ')}`);
  }
  return parts.join(' | ');
}
 ...
 */
export function renderRichHarvest(
  session: SessionState,
  harvestContent: CheckpointContent,
  ...
  let totalTokens = 0;
  const storeTypesUsed = new Set<string>();

  for (const storeKey of Object.keys(session.stores) as Array<keyof SessionStores>) {
  ...
  const sessionStart = session.metadata.createdAt;
  ...
  for (let i = 0; i < timeline.length; i++) {
    ...
    const c = item.entry.content as Record<string, unknown>;
    const absTime = formatAbsoluteTime(item.entry.timestamp);
    const relTime = formatRelativeTime(item.entry.timestamp - sessionStart);
    const rendered = renderEntryContent(item.storeKey, c);

    if (item.storeKey === 'checkpoints') {
      // Checkpoints render inline (not in details) -- they show gate/constraint state
      const headerLine = renderCheckpointHeader(c);
      sections.push(`### Checkpoint (${absTime} | ${relTime})`);
      sections.push(`**Tokens**: ~${item.tokens} (${item.tokenSource})`);
      if (headerLine) sections.push(headerLine);

      // Show constraints added at this checkpoint
      if (Array.isArray(c.addConstraints) && c.addConstraints.length > 0) {
        sections.push('');
        sections.push('**Constraints Added**:');
        for (const ac of c.addConstraints as Array<{ type: string; text: string }>) {
          sections.push(`- [${ac.type}] ${ac.text}`);
        }
      }

      // Show content in details if there's substance
      if (rendered.trim()) {
        sections.push('');
        sections.push(`<details><summary>Checkpoint Content</summary>`);
        sections.push('');
    ...
    } else if (item.storeKey === 'thoughts') {
      // Thoughts are analytical rounds -- primary content
      roundCounter++;
      sections.push(`### Round ${roundCounter} (${absTime} | ${relTime})`);
      sections.push(`**Tokens**: ~${item.tokens} (${item.tokenSource})`);
      sections.push('');
      sections.push(`<details><summary>Analysis</summary>`);
      sections.push('');
      sections.push(rendered);
      sections.push('');
      sections.push('</details>');
      ...
      // All other entry types -- full content in details block
      sections.push(`### ${item.label} (${absTime} | ${relTime})`);
      sections.push(`**Tokens**: ~${item.tokens} (${item.tokenSource})`);
      sections.push('');
      sections.push(`<details><summary>${item.label} Content</summary>`);
      sections.push('');
      sections.push(rendered);
      sections.push('');
      sections.push('</details>');
  ...
  if (session.protocolState && session.protocolState.constraints.size > 0) {
    ...
    for (const [, constraint] of session.protocolState.constraints) {
      const pc = constraint as ProtocolConstraint;
  ...
  for (const cp of allCheckpoints) {
    if (cp.content?.keyFindings) {
      for (const f of cp.content.keyFindings) findingsSet.add(f);
    }
  }
  if (harvestContent.keyFindings) {
    for (const f of harvestContent.keyFindings) findingsSet.add(f);
  }

  if (findingsSet.size > 0) {
    sections.push('## Key Findings');
    sections.push('');
    for (const f of findingsSet) sections.push(`- ${f}`);
    sections.push('');
  }
