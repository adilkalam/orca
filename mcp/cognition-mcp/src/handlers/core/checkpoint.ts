...
import type { CognitionRequest, HandlerResult, CheckpointContent, ProtocolState, FollowUpQuestion, SessionStores } from '../../types.js';
...
function processProtocolState(
  ...
  // Persist sessionFolder on the session when first provided
  if (content.sessionFolder && !session.sessionFolder) {
    session.sessionFolder = content.sessionFolder;
    // Scan existing files to determine starting counter (avoid collision with 00-enter.md, 01-orient.md, etc.)
    try {
      if (existsSync(content.sessionFolder)) {
        const files = readdirSync(content.sessionFolder);
        let maxNum = -1;
        ...
        session.roundFileCounter = maxNum + 1;
      }
    } catch {
      // If scan fails, start at a safe offset
      session.roundFileCounter = 10;
    }
  }
  ...
  const blockingUnknowns = activeConstraints.filter(
    c => c.type === 'BLOCKING_UNKNOWN' && !c.userAsked
  );
...
const STORE_TO_OPERATION: Record<string, string> = {
  thoughts: 'thought',
...
function renderRawJSON(
  ...
  for (const storeKey of Object.keys(session.stores) as Array<keyof SessionStores>) {
    ...
    stores[storeKey] = entries.map(entry => ({
      ...
      request: { operation: operationName, ...entry.content as Record<string, unknown> },
      stored: entry,
    }));
  }

  // Build protocol state for export
  let protocolExport: Record<string, unknown> | undefined;
...
function autoPersistHarvest(
  ...
  content: CheckpointContent,
  _projectPath?: string
): { persisted: boolean; file?: string; sessionFolderFile?: string; followUpQuestions?: FollowUpQuestion[] } | null {
  ...
  try {
    ...
    let harvestFile: string;
    let sessionFolderFile: string | undefined;
