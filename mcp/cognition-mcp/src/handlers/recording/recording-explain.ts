...
import type { SessionNarrative } from '../../recording/types.js';

export async function handleRecordingExplain(
  ...
  for (const cp of checkpoints) {
    ...
    timeline.push(
      `Checkpoint ${cp.id.substring(0, 8)} at ${cp.created_at}: ${summary} (${filesModified} modified, ${filesNew} new).`
    );
  }
  ...
  const rewindEvents = events.filter(e =>
  ...
  let commitInfo: string | null = null;
