import { z } from 'zod';
export function addSegmentTools(server, resend) {
    server.registerTool('create-segment', {
        ...
        if (response.error) {
            throw new Error(`Failed to create segment: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-segments', {
        title: 'List Segments',
...
**When to use:** User says "show my segments", "what lists do I have?", or before create-contact/create-broadcast when segmentId is unknown.`,
        inputSchema: {
            limit: z
                ...
                .max(100)
                .optional()
                .describe('Number of segments to retrieve. Default: 20, Max: 100, Min: 1'),
            after: z
                .string()
                .optional()
        ...
        console.error(`Debug - Listing segments with limit: ${limit}, after: ${after}, before: ${before}`);
        ...
        if (response.error) {
            throw new Error(`Failed to list segments: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('get-segment', {
        title: 'Get Segment',
        ...
        if (response.error) {
            throw new Error(`Failed to get segment: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('remove-segment', {
        ...
        if (response.error) {
            throw new Error(`Failed to remove segment: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
