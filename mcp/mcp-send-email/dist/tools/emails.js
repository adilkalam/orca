...
import { z } from 'zod';
export function addEmailTools(server, resend, { senderEmailAddress, replierEmailAddresses, }) {
...
**Key trigger phrases:** "Send an email", "Email this to", "Notify", "Send a message", "Reply to them", "Schedule an email"`,
        inputSchema: {
            ...
            scheduledAt: z
                .string()
                .optional()
                .describe("Optional parameter to schedule the email. This uses natural language. Examples would be 'tomorrow at 10am' or 'in 2 hours' or 'next day at 9am PST' or 'Friday at 3pm ET'."),
            attachments: z
                .array(z.object({
                filename: z
                    .string()
                ...
                contentType: z
                    .string()
                    .optional()
                    .describe('MIME type (e.g., "application/pdf"). Auto-derived from filename if not set'),
                contentId: z
                    .string()
                    .optional()
                    .describe('Content ID for inline images. Reference in HTML with cid:<contentId>'),
            }))
                .optional()
                .describe('Array of file attachments. Each needs filename plus one of: filePath, url, or content. Max 40MB total.'),
            tags: z
                .array(z.object({
                ...
                .optional()
                .describe('Array of custom tags for tracking/analytics. Each tag has a name and value.'),
            topicId: z
                .string()
                .optional()
                .describe('Topic ID for subscription-based sending. When set, the email respects contact subscription preferences for this topic.'),
    ...
    }, async ({ from, to, subject, text, html, replyTo, scheduledAt, cc, bcc, attachments, tags, topicId, }) => {
        ...
        if (attachments && attachments.length > 0) {
            emailRequest.attachments = await Promise.all(attachments.map(async (att) => {
                const result = {};
                if (att.filename)
    ...
    server.registerTool('list-emails', {
        title: 'List Emails',
...
**Workflow:** list-emails → get-email( id ) when user needs full body or details.`,
        inputSchema: {
            ...
            after: z
                .string()
                .optional()
                .describe('Email ID after which to retrieve more emails (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Email ID before which to retrieve more emails (for backward pagination). Cannot be used with "after".'),
        },
    }, async ({ limit, after, before }) => {
        if (after && before) {
            throw new Error('Cannot use both "after" and "before" parameters. Use only one for pagination.');
        }
        console.error(`Debug - Listing emails with limit: ${limit}, after: ${after}, before: ${before}`);
        ...
        if (response.error) {
            throw new Error(`Failed to list emails: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('get-email', {
        ...
        if (response.error) {
            throw new Error(`Failed to retrieve email: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-received-emails', {
...
**Returns:** Paginated metadata: from, to, subject, received time. Use get-received-email with an ID for full content.`,
        ...
        console.error(`Debug - Listing received emails with limit: ${limit}, after: ${after}, before: ${before}`);
        const paginationOptions = after
            ? { limit, after }
            : before
                ? { limit, before }
        ...
        if (response.error) {
            throw new Error(`Failed to list received emails: ${JSON.stringify(response.error)}`);
        }
        const emails = response.data?.data ?? [];
        const hasMore = response.data?.has_more ?? false;
        if (emails.length === 0) {
        ...
        const emailSummaries = emails
            ...
            const attachmentInfo = attachmentCount > 0 ? ` | Attachments: ${attachmentCount}` : '';
    ...
    server.registerTool('get-received-email', {
        ...
        if (response.error) {
            throw new Error(`Failed to retrieve received email: ${JSON.stringify(response.error)}`);
        }
        const email = response.data;
        if (!email) {
    ...
    server.registerTool('list-received-email-attachments', {
        ...
        inputSchema: {
            ...
            limit: z
                ...
                .max(100)
                .optional()
                .describe('Number of attachments to retrieve. Default: 20, Max: 100, Min: 1'),
            after: z
                .string()
                .optional()
                .describe('Attachment ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Attachment ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
    }, async ({ emailId, limit, after, before }) => {
        if (after && before) {
            throw new Error('Cannot use both "after" and "before" parameters. Use only one for pagination.');
        }
        console.error(`Debug - Listing attachments for received email: ${emailId}`);
        const paginationOptions = after
            ? { emailId, limit, after }
            : before
                ? { emailId, limit, before }
                : { emailId, limit };
        const response = await resend.emails.receiving.attachments.list(paginationOptions);
        if (response.error) {
            throw new Error(`Failed to list attachments: ${JSON.stringify(response.error)}`);
        }
        ...
        const attachmentSummaries = attachments
            .map((att) => `- ${att.filename} | Type: ${att.content_type} | Size: ${att.size} bytes | Download: ${att.download_url} (expires: ${att.expires_at}) | ID: ${att.id}`)
            .join('\n');
        return {
            content: [
                {
    ...
    server.registerTool('get-received-email-attachment', {
        ...
        console.error(`Debug - Getting attachment ${id} for received email: ${emailId}`);
        const response = await resend.emails.receiving.attachments.get({
            emailId,
            id,
        });
        if (response.error) {
            throw new Error(`Failed to retrieve attachment: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('cancel-email', {
        title: 'Cancel Email',
        ...
        if (response.error) {
            throw new Error(`Failed to cancel email: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
                {
    ...
    server.registerTool('update-email', {
        title: 'Update Email',
        ...
        inputSchema: {
            ...
            scheduledAt: z
                .string()
                .describe('The new scheduled time in ISO 8601 format (e.g., "2024-08-05T11:52:01.858Z").'),
        },
    }, async ({ id, scheduledAt }) => {
        console.error(`Debug - Updating email ${id} with scheduledAt: ${scheduledAt}`);
        const response = await resend.emails.update({ id, scheduledAt });
        if (response.error) {
            throw new Error(`Failed to update email: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
                {
    ...
    server.registerTool('list-sent-email-attachments', {
    ...
    server.registerTool('get-sent-email-attachment', {
        ...
        console.error(`Debug - Getting attachment ${id} for sent email: ${emailId}`);
    ...
    server.registerTool('send-batch-emails', {
...
**When to use:** User wants to send many individual emails in bulk (e.g. 50 password resets, 100 receipts). Not for one-to-many broadcasts.`,
        inputSchema: {
            emails: z
                ...
                from: z
                    .email()
                    .optional()
                    .describe('Sender email address. Falls back to the configured default sender if not provided.'),
                replyTo: z
                    .array(z.email())
                    .optional()
                ...
                scheduledAt: z
                    .string()
                    .optional()
                    .describe("Optional schedule time. Uses natural language (e.g., 'tomorrow at 10am') or ISO 8601."),
                tags: z
                    .array(z.object({
                    name: z.string().describe('Tag name (key)'),
                    value: z.string().describe('Tag value'),
        ...
        const emailRequests = emails.map((email) => {
            ...
            if (typeof fromAddress !== 'string') {
                throw new Error(`from address must be provided for email to ${email.to.join(', ')}`);
            }
            const request = {
                to: email.to,
            ...
            if (email.tags && email.tags.length > 0)
                request.tags = email.tags;
            if (email.topicId)
                request.topicId = email.topicId;
            return request;
        });
        const response = await resend.batch.send(emailRequests);
        if (response.error) {
