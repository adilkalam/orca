...
export function addWebhookTools(server, resend) {
    server.registerTool('create-webhook', {
        ...
        console.error(`Debug - Creating webhook for endpoint: ${endpoint} with events: ${events.join(', ')}`);
        const response = await resend.webhooks.create({ endpoint, events });
        if (response.error) {
            throw new Error(`Failed to create webhook: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-webhooks', {
        ...
        if (response.error) {
            throw new Error(`Failed to list webhooks: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('get-webhook', {
        ...
        if (response.error) {
            throw new Error(`Failed to get webhook: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('update-webhook', {
        ...
        if (response.error) {
            throw new Error(`Failed to update webhook: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('remove-webhook', {
        ...
        if (response.error) {
            throw new Error(`Failed to remove webhook: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
