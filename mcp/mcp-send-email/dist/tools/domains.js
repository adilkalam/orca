...
function formatDnsRecords(records) {
    ...
    return records
        .map((r) => `${r.record} (${r.type}):\n  Name: ${r.name}\n  Value: ${r.value}\n  TTL: ${r.ttl}\n  Status: ${r.status}${r.priority !== undefined ? `\n  Priority: ${r.priority}` : ''}`)
        .join('\n\n');
}
export function addDomainTools(server, resend) {
    server.registerTool('create-domain', {
        ...
        inputSchema: {
            ...
            tls: z
                ...
                .optional()
                .describe('TLS mode. "opportunistic" attempts secure connection with fallback. "enforced" requires TLS or fails. Defaults to "opportunistic".'),
            capabilities: z
                .object({
                sending: z
    ...
    }, async ({ name, region, customReturnPath, openTracking, clickTracking, tls, capabilities, }) => {
    ...
    server.registerTool('list-domains', {
        ...
        inputSchema: {
            ...
            after: z
                .string()
                .optional()
                .describe('Domain ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Domain ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
    }, async ({ limit, after, before }) => {
        if (after && before) {
            throw new Error('Cannot use both "after" and "before" parameters. Use only one for pagination.');
        }
        console.error(`Debug - Listing domains with limit: ${limit}, after: ${after}, before: ${before}`);
    ...
    server.registerTool('update-domain', {
        title: 'Update Domain',
        ...
        inputSchema: {
            ...
            tls: z
                .enum(['opportunistic', 'enforced'])
                .optional()
                .describe('TLS mode. "opportunistic" attempts secure connection with fallback. "enforced" requires TLS or fails.'),
            capabilities: z
                .object({
                sending: z
                    .enum(['enabled', 'disabled'])
                ...
                .optional()
                .describe('Domain capabilities. At least one capability must remain enabled.'),
        },
    ...
    server.registerTool('remove-domain', {
        ...
        if (response.error) {
            throw new Error(`Failed to remove domain: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('verify-domain', {
        ...
        if (response.error) {
            throw new Error(`Failed to verify domain: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
                {
