import { z } from 'zod';
export function addContactPropertyTools(server, resend) {
    server.registerTool('create-contact-property', {
        ...
        inputSchema: {
            key: z
                .string()
                .nonempty()
                .describe('The property key. Max 50 characters, only alphanumeric characters and underscores allowed.'),
            type: z
                .enum(['string', 'number'])
            ...
            fallbackValue: z
                ...
                .optional()
                .describe('Default value when the property is not set for a contact. Must match the specified type.'),
        },
    }, async ({ key, type, fallbackValue }) => {
        console.error(`Debug - Creating contact property with key: ${key}, type: ${type}`);
        ...
        if (response.error) {
            throw new Error(`Failed to create contact property: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-contact-properties', {
        ...
        inputSchema: {
            limit: z
                ...
                .max(100)
                .optional()
                .describe('Number of contact properties to retrieve. Default: 20, Max: 100, Min: 1'),
            after: z
                .string()
                .optional()
                .describe('Contact property ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Contact property ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
    }, async ({ limit, after, before }) => {
        if (after && before) {
            throw new Error('Cannot use both "after" and "before" parameters. Use only one for pagination.');
        }
        console.error(`Debug - Listing contact properties with limit: ${limit}, after: ${after}, before: ${before}`);
        ...
        if (response.error) {
            throw new Error(`Failed to list contact properties: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('get-contact-property', {
        ...
        console.error(`Debug - Getting contact property with id: ${contactPropertyId}`);
        const response = await resend.contactProperties.get(contactPropertyId);
        if (response.error) {
            throw new Error(`Failed to get contact property: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('update-contact-property', {
        ...
        inputSchema: {
            ...
            fallbackValue: z
                ...
                .describe('New default value for the property. Pass null to remove the fallback value. Must match the property type.'),
        },
    }, async ({ contactPropertyId, fallbackValue }) => {
        console.error(`Debug - Updating contact property with id: ${contactPropertyId}`);
        const response = await resend.contactProperties.update({
            id: contactPropertyId,
            fallbackValue,
        });
        if (response.error) {
            throw new Error(`Failed to update contact property: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('remove-contact-property', {
        ...
        console.error(`Debug - Removing contact property with id: ${contactPropertyId}`);
        const response = await resend.contactProperties.remove(contactPropertyId);
        if (response.error) {
            throw new Error(`Failed to remove contact property: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
