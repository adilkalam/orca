import { z } from 'zod';
export function addBroadcastTools(server, resend, { senderEmailAddress, replierEmailAddresses, }) {
    server.registerTool('create-broadcast', {
...
**Workflow:** list-audiences (if needed) → create-broadcast → send-broadcast( id ). Optionally update-broadcast before sending.`,
        inputSchema: {
            name: z
                .string()
                .nonempty()
                .describe('Name for the broadcast. If the user does not provide a name, go ahead and create a descriptive name for them, based on the email subject/content and the context of your conversation.'),
            audienceId: z.string().nonempty().describe('Audience ID to send to'),
            subject: z.string().nonempty().describe('Email subject'),
            text: z
                .string()
                .nonempty()
                .describe('Plain text version of the email content. The following placeholders may be used to personalize the email content: {{{FIRST_NAME|fallback}}}, {{{LAST_NAME|fallback}}}, {{{EMAIL}}}, {{{RESEND_UNSUBSCRIBE_URL}}}'),
            html: z
                .string()
                .optional()
                .describe('HTML version of the email content. The following placeholders may be used to personalize the email content: {{{FIRST_NAME|fallback}}}, {{{LAST_NAME|fallback}}}, {{{EMAIL}}}, {{{RESEND_UNSUBSCRIBE_URL}}}'),
            previewText: z
                .string()
                .optional()
    ...
    }, async ({ name, audienceId, subject, text, html, previewText, from, replyTo, }) => {
        console.error(`Debug - Creating broadcast: ${name ?? '<no-name>'} to audience: ${audienceId}`);
        ...
        if (response.error) {
            throw new Error(`Failed to create broadcast: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('send-broadcast', {
...
**Workflow:** create-broadcast → send-broadcast( id ). Use list-broadcasts to find existing draft/sent broadcasts.`,
        inputSchema: {
            ...
            scheduledAt: z
                .string()
                .optional()
                .describe('When to send the broadcast. Value may be in ISO 8601 format (e.g., 2024-08-05T11:52:01.858Z) or in natural language (e.g., "tomorrow at 10am", "in 2 hours", "next day at 9am PST", "Friday at 3pm ET"). If not provided, the broadcast will be sent immediately.'),
        },
        ...
        if (response.error) {
            throw new Error(`Failed to send broadcast: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('list-broadcasts', {
...
**When to use:** User asks "show my broadcasts", "what newsletters did I send?", "list campaigns". Use get-broadcast for full details of one.`,
        ...
        if (response.error) {
            throw new Error(`Failed to list broadcasts: ${JSON.stringify(response.error)}`);
        }
        ...
        return {
            content: [
                ...
                ...broadcasts.map(({ name, id, audience_id, status, created_at, scheduled_at, sent_at, }) => ({
                    type: 'text',
                    text: [
                        `ID: ${id}`,
                        `Name: ${name}`,
    ...
    server.registerTool('get-broadcast', {
        title: 'Get Broadcast',
        ...
        if (response.error) {
            throw new Error(`Failed to get broadcast: ${JSON.stringify(response.error)}`);
        }
        const { id: broadcastId, name, audience_id, from, subject, reply_to, preview_text, status, created_at, scheduled_at, sent_at, } = response.data;
        return {
            content: [
                {
                    type: 'text',
    ...
    server.registerTool('remove-broadcast', {
        ...
        if (response.error) {
            throw new Error(`Failed to remove broadcast: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('update-broadcast', {
    ...
    }, async ({ id, name, audienceId, from, html, text, subject, replyTo, previewText, }) => {
        ...
        if (response.error) {
            throw new Error(`Failed to update broadcast: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
