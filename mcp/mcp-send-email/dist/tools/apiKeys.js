import { z } from 'zod';
export function addApiKeyTools(server, resend) {
    server.registerTool('create-api-key', {
        ...
        inputSchema: {
            ...
            permission: z
                ...
                .optional()
                .describe('Access level. "full_access" grants complete resource management. "sending_access" restricts to email delivery only.'),
            domainId: z
                .string()
                .optional()
                .describe('Restrict API key to send emails from a specific domain. Only applicable when permission is "sending_access".'),
        },
        ...
        if (response.error) {
            throw new Error(`Failed to create API key: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-api-keys', {
        ...
        inputSchema: {
            ...
            after: z
                .string()
                .optional()
                .describe('API key ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('API key ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
    }, async ({ limit, after, before }) => {
        if (after && before) {
            throw new Error('Cannot use both "after" and "before" parameters. Use only one for pagination.');
        }
        console.error(`Debug - Listing API keys with limit: ${limit}, after: ${after}, before: ${before}`);
        ...
        if (response.error) {
            throw new Error(`Failed to list API keys: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('remove-api-key', {
        ...
        if (response.error) {
            throw new Error(`Failed to remove API key: ${JSON.stringify(response.error)}`);
        }
        return {
