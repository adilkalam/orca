import { z } from 'zod';
export function addTopicTools(server, resend) {
    server.registerTool('create-topic', {
        ...
        inputSchema: {
            ...
            defaultSubscription: z
                .enum(['opt_in', 'opt_out'])
                .describe('Default subscription preference for new contacts. Cannot be modified after creation.'),
            description: z
                .string()
                .max(200)
                .optional()
        ...
        if (response.error) {
            throw new Error(`Failed to create topic: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-topics', {
        ...
        if (response.error) {
            throw new Error(`Failed to list topics: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('update-topic', {
        title: 'Update Topic',
        ...
        if (response.error) {
            throw new Error(`Failed to update topic: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('remove-topic', {
        title: 'Remove Topic',
        ...
        if (response.error) {
            throw new Error(`Failed to remove topic: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
