import { z } from 'zod';
export function addContactTools(server, resend) {
    server.registerTool('create-contact', {
        ...
        inputSchema: {
            ...
            properties: z
                ...
                .optional()
                .describe('Custom property key-value pairs for the contact (e.g. { "company_name": "Acme" })'),
            segmentIds: z
                .array(z.string())
                .optional()
    ...
    }, async ({ email, firstName, lastName, unsubscribed, properties, segmentIds, topics, }) => {
        ...
        if (response.error) {
            throw new Error(`Failed to create contact: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-contacts', {
        title: 'List Contacts',
...
**When to use:** User asks "who's in this list?", "show contacts", "who did I add?" Don't bother telling the user the IDs, unsubscribe statuses, or creation dates unless they ask for them.`,
        inputSchema: {
            segmentId: z
                .string()
                .optional()
                .describe('Segment ID to filter by. If provided, only contacts in this segment will be returned.'),
            limit: z
                .number()
                .min(1)
                .max(100)
                .optional()
                .describe('Number of contacts to retrieve. Default: 20, Max: 100, Min: 1'),
            after: z
                .string()
                .optional()
                .describe('Contact ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Contact ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
    }, async ({ segmentId, limit, after, before }) => {
        if (after && before) {
            throw new Error('Cannot use both "after" and "before" parameters. Use only one for pagination.');
        }
        console.error(`Debug - Listing contacts with segmentId: ${segmentId}, limit: ${limit}, after: ${after}, before: ${before}`);
        const options = {};
        if (segmentId)
            options.segmentId = segmentId;
        if (limit !== undefined)
            options.limit = limit;
        if (after)
            options.after = after;
        ...
        const response = await resend.contacts.list(Object.keys(options).length > 0 ? options : undefined);
        if (response.error) {
            throw new Error(`Failed to list contacts: ${JSON.stringify(response.error)}`);
        }
        ...
        return {
            content: [
                ...
                ...contacts.map((contact) => {
                    const props = contact
                        .properties;
                    const propsLine = props && Object.keys(props).length > 0
    ...
    server.registerTool('get-contact', {
        title: 'Get Contact',
        ...
        if (id) {
            ...
            throw new Error('You must provide either `id` or `email` to get a contact.');
        }
        if (response.error) {
            throw new Error(`Failed to get contact: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('update-contact', {
        ...
        inputSchema: {
            ...
            firstName: z
                ...
                .nullable()
                .optional()
                .describe("Contact first name. Pass `null` to remove the contact's first name."),
            lastName: z
                .string()
                .nullable()
                .optional()
                .describe("Contact last name. Pass `null` to remove the contact's last name."),
            unsubscribed: z
                .boolean()
                .optional()
                .describe('Whether the contact is unsubscribed from all broadcasts'),
            properties: z
                .record(z.string(), z.union([z.string(), z.number(), z.null()]))
                .optional()
                .describe('Custom property key-value pairs to update (e.g. { "company_name": "Acme" })'),
        },
        ...
        if (id) {
            ...
            throw new Error('You must provide either `id` or `email` to update a contact.');
        }
        if (response.error) {
            throw new Error(`Failed to update contact: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('remove-contact', {
        ...
        if (id) {
            ...
            throw new Error('You must provide either `id` or `email` to remove a contact.');
        }
        if (response.error) {
            throw new Error(`Failed to remove contact: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('add-contact-to-segment', {
        ...
        console.error(`Debug - Adding contact (id: ${contactId}, email: ${email}) to segment: ${segmentId}`);
        let response;
        if (contactId) {
            ...
            throw new Error('You must provide either `contactId` or `email` to add a contact to a segment.');
        }
        if (response.error) {
            throw new Error(`Failed to add contact to segment: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('remove-contact-from-segment', {
        ...
        console.error(`Debug - Removing contact (id: ${contactId}, email: ${email}) from segment: ${segmentId}`);
        let response;
        if (contactId) {
            ...
            throw new Error('You must provide either `contactId` or `email` to remove a contact from a segment.');
        }
        if (response.error) {
            throw new Error(`Failed to remove contact from segment: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
    ...
    server.registerTool('list-contact-segments', {
        ...
        inputSchema: {
            ...
            limit: z
                ...
                .max(100)
                .optional()
                .describe('Number of segments to retrieve. Max: 100, Min: 1. If omitted, all segments are returned.'),
            after: z
                .string()
                .optional()
                .describe('Segment ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Segment ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
        ...
        if (!contactId && !email) {
            throw new Error('You must provide either `contactId` or `email` to list contact segments.');
        }
        console.error(`Debug - Listing segments for contact (id: ${contactId}, email: ${email})`);
        ...
        if (response.error) {
            throw new Error(`Failed to list contact segments: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('list-contact-topics', {
        ...
        inputSchema: {
            ...
            after: z
                .string()
                .optional()
                .describe('Topic ID after which to retrieve more (for forward pagination). Cannot be used with "before".'),
            before: z
                .string()
                .optional()
                .describe('Topic ID before which to retrieve more (for backward pagination). Cannot be used with "after".'),
        },
        ...
        if (!id && !email) {
            throw new Error('You must provide either `id` or `email` to list contact topics.');
        }
        console.error(`Debug - Listing topics for contact (id: ${id}, email: ${email})`);
        ...
        if (response.error) {
            throw new Error(`Failed to list contact topics: ${JSON.stringify(response.error)}`);
        }
    ...
    server.registerTool('update-contact-topics', {
        ...
        if (!id && !email) {
            throw new Error('You must provide either `id` or `email` to update contact topics.');
        }
        console.error(`Debug - Updating topics for contact (id: ${id}, email: ${email})`);
        const contactField = id ? { id } : { email: email };
        ...
        if (response.error) {
            throw new Error(`Failed to update contact topics: ${JSON.stringify(response.error)}`);
        }
        return {
            content: [
                {
