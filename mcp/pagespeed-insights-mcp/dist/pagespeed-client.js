...
export class PageSpeedClient {
    ...
    async makeRequest(url, correlationId) {
    ...
    async analyzePageSpeed(input, correlationId) {
        const logger = createRequestLogger(correlationId, "analyze-page-speed");
        return this.limiter(async () => {
            const cacheKey = createPSICacheKey(input.url, input.strategy, input.category || ["performance"], input.locale);
            // Try cache first
            ...
            if (input.category && input.category.length > 0) {
                input.category.forEach((cat) => url.searchParams.append("category", cat));
            }
    ...
    async getCruxData(input, correlationId) {
...
//# sourceMappingURL=pagespeed-client.js.map