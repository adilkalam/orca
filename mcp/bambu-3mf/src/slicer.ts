...
let cachedCliPath: string | false | null = null;
 ...
 */
export function findOrcaSlicerCli(): string | null {
...
export function slicerNotFoundError(): {
  ...
  return {
    ...
    install_guide:
      'After installing, the CLI is typically at: /Applications/OrcaSlicer.app/Contents/MacOS/OrcaSlicer',
 ...
 */
export async function runSlicer(
  filePath: string,
  outputDir?: string,
  profileIni?: string,
  timeout: number = SLICER_TIMEOUT
 ...
 */
export async function parseSlicerOutput(
  outputDir: string,
  stdout: string,
  stderr: string
): Promise<SliceMetrics> {
  ...
  const costUsd =
    weightGrams !== null ? Math.round(weightGrams * DEFAULT_COST_PER_GRAM * 100) / 100 : null;
...
function extractFromJson(
  ...
  const timeMinutes =
    (data.estimated_time_minutes as number) ??
    (data.time_minutes as number) ??
    null;
  const weightGrams =
    (data.filament_weight_grams as number) ??
    (data.weight_grams as number) ??
    null;
  const lengthMeters =
    (data.filament_length_meters as number) ??
    (data.length_meters as number) ??
    null;
...
export function extractTimeFromText(text: string): number | null {
  const patterns: Array<{ re: RegExp; handler: (m: RegExpMatchArray) => number }> = [
    {
...
function extractLengthFromText(text: string): number | null {
  const patterns = [
...
export function formatTimeDelta(minutes: number): string {
  ...
  if (hours > 0 && mins > 0) return `${sign}${hours}h ${mins}m`;
 ...
 */
export function jsonPresetToIni(preset: Settings): string {
