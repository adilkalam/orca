 ...
 */
export async function readOrcaConfig(
  path: string,
  keys?: string[]
): Promise<OrcaConfig> {
      ...
      switch (mappedField) {
        ...
        case 'wall_loops': {
          if (xmlValue) {
            ...
            if (!isNaN(loops)) result.wall_loops = loops;
          }
          break;
        }
...
function filterKeys(config: OrcaConfig, keys?: string[]): OrcaConfig {
  if (!keys || keys.length === 0) return config;

  const filtered: OrcaConfig = {
    filament_type: null,
    nozzle_diameter: null,
    layer_height: null,
    infill_density: null,
  ...
  for (const key of keys) {
    if (key in config && key !== 'raw') {
      (filtered as unknown as Record<string, unknown>)[key] = (config as unknown as Record<string, unknown>)[key];
    }
...
function extractTimeEstimate(text: string): string | null {
  ...
  for (const pattern of patterns) {
    ...
    if (match) {
      ...
      const val = match[1];
      if (/h/i.test(match[0])) return `${val}h`;
      return `${val}m`;
    }
  }
