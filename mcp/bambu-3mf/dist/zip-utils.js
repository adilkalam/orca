 ...
 */
export async function read3MF(path) {
    if (!existsSync(path)) {
 ...
 */
export async function write3MF(originalPath, contents, outputPath) {
    ...
    contents.zip.file('Metadata/project_settings.config', JSON.stringify(contents.projectSettings, null, 4));
    for (const [slotStr, settings] of Object.entries(contents.filamentSettings)) {
        contents.zip.file(`Metadata/filament_settings_${slotStr}.config`, JSON.stringify(settings, null, 4));
    }
...
export function stripGcodeKeys(settings) {
    const result = {};
...
export function isGcodeKey(key) {
    return GCODE_KEYS.includes(key);
}
/** Validate that no gcode keys are present. Throws if any found. */
export function guardGcodeKeys(settings) {
    const violations = Object.keys(settings).filter(isGcodeKey);
    if (violations.length > 0) {
        throw new Error(`FORBIDDEN: Cannot modify gcode keys: ${violations.join(', ')}. ` +
 ...
 */
export function mergeIntoProjectSettings(projectSettings, updates, filamentSlot, filamentCount) {
    ...
    for (const [key, value] of Object.entries(updates)) {
        ...
        if (Array.isArray(value)) {
            ...
            if (Array.isArray(existingValue)) {
                ...
                while (arr.length < filamentCount)
 ...
 * NEVER touches gcode keys.
 */
export function mergeIntoFilamentSettings(filamentSettings, updates, filamentSlot) {
    guardGcodeKeys(updates);
