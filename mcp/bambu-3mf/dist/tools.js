...
import { findOrcaSlicerCli, slicerNotFoundError, runSlicer, formatTime, formatTimeDelta, jsonPresetToIni, } from './slicer.js';
 ...
 */
export async function sliceAnalyze(path) {
 ...
 */
export async function sliceCompare(path, presets) {
    if (!findOrcaSlicerCli()) {
        return slicerNotFoundError();
    }
    const absPath = resolve(path);
    ...
    for (const presetName of presetNames) {
        ...
        try {
            ...
            if (current.filament_weight_grams !== null &&
    ...
    results.recommendation = generateRecommendation(current, results);
    return results;
}
/**
 ...
 */
export async function sliceBatch(path, quantity, preset) {
    if (!findOrcaSlicerCli()) {
        return slicerNotFoundError();
    }
    ...
    const totalTimeFormatted = days > 0
    ...
    return {
        ...
        total_filament_kg: weightPerUnit !== null ? Math.round((weightPerUnit * quantity) / 1000 * 100) / 100 : null,
        total_cost_usd: costPerUnit !== null ? Math.round(costPerUnit * quantity * 100) / 100 : null,
 ...
 */
export async function readOrcaConfigTool(path, keys) {
...
async function getAvailablePresetNames() {
    const names = [];
    const dirs = [
        join(DEFAULT_PRESETS_DIR, 'filaments'),
        join(DEFAULT_PRESETS_DIR, 'process'),
 ...
 */
async function createTempIniFromPreset(presetName) {
...
function generateRecommendation(current, results) {
    if (current.estimated_time_minutes === null) {
    ...
    for (const [name, metrics] of Object.entries(results)) {
        if (name === 'current' || name === 'recommendation')
            continue;
        if (metrics &&
    ...
    if (savings > 30) {
        return (`Recommendation: Use ${fastestName} profile. ` +
            `Saves ${formatTimeDelta(-savings).replace(/^[+-]/, '')} per unit compared to current settings.`);
    }
    if (savings > 0) {
        return (`All profiles have similar print times. ` +
