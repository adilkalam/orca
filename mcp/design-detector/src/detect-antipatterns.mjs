...
if (!IS_BROWSER) {
  try {
    const rulesPath = process.env.DESIGN_COLLECTION_PATH
      ? path.join(process.env.DESIGN_COLLECTION_PATH, 'detector-rules.json')
      : '/Users/adilkalam/ORCA-OS/docs/concepts/design-contract/detector-rules.json';
    if (fs.existsSync(rulesPath)) {
      EXTERNAL_RULES = JSON.parse(fs.readFileSync(rulesPath, 'utf-8'));
    }
  } catch (_err) {
    // Silent fallback — inline Bakaus rules still run.
  }
}
...
// ─── External rules runner ──────────────────────────────────────────────────
//
// Applies EXTERNAL_RULES loaded from design-contract/detector-rules.json.
// Pattern-based rules only. Handler-based rules (requires_handler: true) are
// skipped here — they live in src/detectors/index.ts and require a separate
// compile/import step which is deferred.
...
function runExternalRules(content, filePath) {
  const findings = [];
  const rules = (EXTERNAL_RULES && EXTERNAL_RULES.rules) || [];
  for (const rule of rules) {
    if (rule.requires_handler) {
      // Handler-based rule — skip (deferred until handlers are compiled/imported).
      continue;
    }
    for (const pattern of rule.patterns || []) {
      ...
      // `target` is either "css" or "source"; pattern-based rules operate on
      // the raw file text in both cases. Bakaus's engine already extracts
      // <style> blocks for SFCs via the regex-matcher path, but for external
      // rules we keep it simple and scan the full content.
      const target = content;
      const matches = [...target.matchAll(regex)];
      for (const match of matches) {
        const line = getLineNumber(target, match.index);
        const snippet = String(match[0]).slice(0, 200);
        findings.push(externalFinding(rule, filePath, snippet, line));
      }
    }
  }
  return findings;
...
function detectText(content, filePath) {
  ...
  // Apply EXTERNAL_RULES (Adil's seeded JSON rules from design-contract/detector-rules.json).
  // Handler-based rules are deferred — only pattern-based rules run here.
  findings.push(...runExternalRules(content, filePath));
