...
export class WorkshopClient {
     ...
     */
    async pinRecentEntries() {
        const db = this.writeDatabase();
        if (!db)
            return;
        try {
            // Find preference/antipattern entries without pinned metadata
            const rows = db.prepare("SELECT id, entry_metadata FROM entries WHERE type IN ('preference', 'antipattern')").all();
    ...
    async prune() {
        ...
        if (!db)
            return;
        try {
            const countRow = db.prepare('SELECT count(*) as cnt FROM entries').get();
            if (countRow.cnt <= maxEntries)
                return;
            const rows = db.prepare('SELECT id, timestamp, entry_metadata FROM entries').all();
            ...
            console.error(`[WorkshopClient] Pruned ${idsToDelete.length} entries (count was ${countRow.cnt}, max is ${maxEntries})`);
        }
        catch (error) {
    ...
    async saveTaskHistory(task) {
        ...
        await this.runWorkshop(`note "${noteText}" -t task-history -t "${task.domain}"`);
        await this.prune();
    }
    /**
    ...
    async queryTaskHistory(query, limit = 10) {
      ...
      `;
            ...
            let results = rows.map(row => {
                ...
                const filesFromContent = filesMatch ? filesMatch[1].split(',').map((f) => f.trim()).filter(Boolean) : [];
