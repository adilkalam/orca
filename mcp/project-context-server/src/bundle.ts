...
export class ContextBundler {
  ...
  private codeSearchCache: Map<string, CodeSearch> = new Map();

  constructor(private semantic: SemanticSearch) {
   ...
   */
  private getCodeSearch(projectPath: string): CodeSearch {
   ...
   */
  private async getProjectStateSummary(projectPath: string): Promise<ProjectStateSummary> {
    const statePath = join(projectPath, '.claude', 'memory', 'state.json');

    let fullState: ProjectState | null = null;
   ...
   */
  private summarizeProjectState(state: ProjectState, projectPath: string): ProjectStateSummary {
    // Count files in tree
    ...
    const topLevelDirs = (state.fileStructure.children || [])
      .filter((c: FileNode) => c.type === 'directory')
      .map((c: FileNode) => c.name)
    ...
    const componentNames = state.components
      ...
      .map((c: ComponentRegistry) => c.name);
   ...
   */
  private extractKeyDependencies(deps: Record<string, string>): string[] {
    const keyPatterns = [
   ...
   * Target: <200 tokens
   */
  private async getDesignSystemSummary(projectPath: string) {
   ...
   */
  private extractPrimaryColors(system: any): string[] {
    const colors: string[] = [];

    // Try different common structures
    if (system.colors?.primary) colors.push(`primary: ${system.colors.primary}`);
    if (system.colors?.secondary) colors.push(`secondary: ${system.colors.secondary}`);
    if (system.colors?.accent) colors.push(`accent: ${system.colors.accent}`);
    if (system.colors?.background) colors.push(`bg: ${system.colors.background}`);
    if (system.colors?.text) colors.push(`text: ${system.colors.text}`);

    // Or it might be flat
    if (system.primary) colors.push(`primary: ${system.primary}`);
    if (system.background) colors.push(`bg: ${system.background}`);
   ...
   */
  private countFilesInTree(node: FileNode): number {
    if (node.type === 'file') return 1;
    return (node.children || []).reduce(
      (sum: number, child: FileNode) => sum + this.countFilesInTree(child),
      0
    );
  }
}
