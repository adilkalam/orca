...
import {
  ...
  getCheckpoint,
  countEventsBetween,
} from '../../recording/db.js';
import type { CheckpointComparison } from '../../recording/types.js';

export async function handleRecordingCompare(
  ...
  const sessionId = cpA.session_id === cpB.session_id
    ? cpA.session_id
    : cpA.session_id;
  const eventsBetween = countEventsBetween(
    db,
    sessionId,
    cpA.created_at,
    cpB.created_at
 ...
 */
function countArrayDiff(
  snapA: Record<string, unknown>,
  snapB: Record<string, unknown>,
  key: string
): number {
  const countA = Array.isArray(snapA[key]) ? (snapA[key] as unknown[]).length : 0;
  const countB = Array.isArray(snapB[key]) ? (snapB[key] as unknown[]).length : 0;
