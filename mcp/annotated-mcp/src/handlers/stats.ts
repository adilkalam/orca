...
const FILE_TO_OPERATION: Record<string, string> = {
 ...
 */
async function getSessionIds(projectPath?: string): Promise<string[]> {
 ...
 */
async function loadSessionData(sessionId: string, deep: boolean, projectPath?: string): Promise<SessionData | null> {
  const sessionsDir = resolveSessionsDir(projectPath);
 ...
 */
function computeOverview(sessions: SessionData[]) {
  const operationFrequency: Record<string, number> = {};
  ...
  for (const session of sessions) {
    ...
    if (session.createdAt) {
      if (session.createdAt < firstDate) firstDate = session.createdAt;
      if (session.createdAt > lastDate) lastDate = session.createdAt;
 ...
 */
function computeReflexDistribution(sessions: SessionData[]) {
  const reflexCounts: Record<string, number> = {};
  let totalReflexes = 0;
  let caughtCount = 0;

  for (const session of sessions) {
    for (const entry of session.entries) {
      const reflexes = entry.content.reflexesObserved as Array<{
        reflex: string;
        caught: boolean;
      }> | undefined;
  ...
  return {
    reflexDistribution: {
      dataType: 'self_assessed' as const,
 ...
 */
function computeCounterfactualGaps(sessions: SessionData[]) {
  const gaps: Array<{ session: string; gap: string; timestamp: number }> = [];

  for (const session of sessions) {
    for (const entry of session.entries) {
      const cf = entry.content.defaultCounterfactual as {
        gap?: string;
 ...
 */
export async function handleReasoningStats(
  ...
  try {
    ...
    const allSessions = (await Promise.all(sessionDataPromises)).filter(
