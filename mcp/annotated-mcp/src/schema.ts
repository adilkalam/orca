...
const SCHEMA_HINTS: Record<string, string> = {
  thought: 'Required: {thought: string, thoughtNumber: number (default 1), totalThoughts: number (default 1), nextThoughtNeeded: boolean (default true)}',
  systems: 'Required: {system: string, components: [{name, function}], relationships: [{from, to, type}], feedbackLoops: string[] (objects auto-coerced)}',
  causal_analysis: 'Required: {phenomenon: string, causes: [{factor, type, strength}], effects: [{outcome, likelihood, timeframe}], chains: [{sequence: string[], probability: number}] (strings/objects auto-normalized)}',
  collaborative_reasoning: 'Required: {topic: string, perspectives: [{role, viewpoint, arguments}], tensions: string[] (objects auto-coerced)}',
  decide: 'Required: {statement: string, options: [{name, description}], criteria: string[], analysis: string, choice: string}',
  mental_model: 'Optional all: {modelName, problem, steps: string[], reasoning, conclusion, setup, rootCauses: [{failure, cause, preventable}]}',
  tree_of_thought: 'Required: {branches: [{id, thought}], bestPath: string[], pruned: string[]}',
  ulysses_protocol: 'Required: {goal: string, temptations: [{trigger, temptation, risk}], commitments: [{commitment, enforcement, consequences}], safeguards: [{safeguard, trigger}]}',
  orchestration_suggest: 'Required: {task: string, complexity: simple|medium|complex, suggestedOperations: [{operation, reason, order}], alternativeApproaches: [{approach, tradeoffs}], recommendation: string}',
};
