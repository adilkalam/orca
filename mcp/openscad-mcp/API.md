...
```json
{
  ...
  "result": {
    "tools": [
      ...
      {
        "name": "analyze_stl",
        "description": "Analyze STL geometry: dimensions, volume, cross-sections, wall thickness",
        "inputSchema": { ... }
      },
      {
        "name": "compare_stl",
        "description": "Compare two STL files with boolean difference",
        "inputSchema": { ... }
      }
    ]
  }
...
```typescript
interface AnalyzeStlRequest {
  stl_path: string;              // Absolute path to STL file (required)
  measurements?: string[];       // Profiles: "basics", "thickness", "cross_sections"
                                 // Default: ["basics"]
  cross_section_axis?: string;   // "x", "y", "z", or "all" (default: "all")
  cross_section_heights?: number[]; // Explicit heights in mm
                                    // Default: auto-slice at 25%, 50%, 75%
  thickness_method?: string;     // "ray" (default)
}
```
...
interface AnalyzeStlResponse {
  ...
  basics?: {
    ...
    vertex_count: number;
    euler_number: number;
    genus: number | null;           // null when not watertight
  };
  thickness?: {
    min_mm: number | null;
    max_mm: number | null;
    mean_mm: number | null;
    percentile_5_mm: number | null;
    percentile_95_mm: number | null;
    method: string;
    sample_count: number;
    warning?: string;
  };
...
```python
# Basic analysis
result = await client.call_tool("analyze_stl", {
    "stl_path": "/path/to/model.stl"
})

# Full analysis with all profiles
result = await client.call_tool("analyze_stl", {
    "stl_path": "/path/to/model.stl",
    "measurements": ["basics", "thickness", "cross_sections"],
    "cross_section_axis": "z",
    "cross_section_heights": [5.0, 10.0, 15.0]
})
```
...
|------------|-----------|
| `FILE_NOT_FOUND` | STL path does not exist |
| `INVALID_STL` | File exists but cannot be parsed as STL |
| `INVALID_PARAMETERS` | Invalid measurement profile or axis |

### compare_stl

Compare two STL files to quantify dimensional and volumetric differences. Uses boolean difference via manifold3d when both meshes are watertight.

#### Request Schema

```typescript
interface CompareStlRequest {
  reference_path: string;  // Absolute path to reference STL (required)
  generated_path: string;  // Absolute path to generated STL (required)
}
```
...
```typescript
interface CompareStlResponse {
  reference: string;
  generated: string;
  dimension_comparison: {
    reference_dimensions_mm: [number, number, number];
    generated_dimensions_mm: [number, number, number];
    difference_mm: [number, number, number];
    max_dimension_error_mm: number;
  };
  volume_comparison: {
    reference_volume_mm3: number | null;
    generated_volume_mm3: number | null;
    difference_mm3: number | null;
    difference_percent: number | null;
  };
  boolean_difference: {
    excess_volume_mm3: number;    // Material in generated but not reference
    missing_volume_mm3: number;   // Material in reference but not generated
    method: string;
  } | {
    warning: string;              // When non-watertight or timed out
    method?: string;
  };
  topology: {
    reference_watertight: boolean;
    generated_watertight: boolean;
    both_watertight: boolean;
  };
}
```
...
```python
# Compare reference to generated STL
result = await client.call_tool("compare_stl", {
    "reference_path": "/path/to/reference.stl",
    "generated_path": "/path/to/output.stl"
})

# Check dimensional accuracy
diff = result["dimension_comparison"]
if diff["max_dimension_error_mm"] > 0.5:
    print(f"Warning: max error {diff['max_dimension_error_mm']}mm")
```
...
| Error Code | Condition |
|------------|-----------|
| `FILE_NOT_FOUND` | Either STL path does not exist |
| `INVALID_STL` | Either file cannot be parsed as STL |

#### Non-Watertight Fallback

When either mesh is not watertight, `boolean_difference` contains a warning message instead of volume data. Dimension and volume comparison are always included regardless.
...
**API Version:** 1.1.0 | **Last Updated:** 2026-02-20