...
def manage_response_size(
    ...
    if output_dir is None:
        # Prefer project's .claude/renders/ if it exists (or can be created)
        cwd_renders = Path.cwd() / ".claude" / "renders"
        try:
            cwd_renders.mkdir(parents=True, exist_ok=True)
            output_dir = cwd_renders
        except OSError:
...
async def render_single(
    ...
    variables: Optional[Dict[str, Any]] = None,
    auto_center: bool = False,
    output_format: Optional[str] = "file_path",
    ...
    Args:
        ...
        output_format: Output format - "auto", "base64", "file_path", or "compressed" (default: "file_path")
...
# ============================================================================
# Import analysis module to register analyze_stl and compare_stl tools
# on the shared mcp instance. Must be after mcp is defined.
