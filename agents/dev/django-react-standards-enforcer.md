...
|----------|-----------------|--------------|----------|
| Critical | -15 to -25 | [Critical] | Security vulnerability, missing auth, `any` on API response |
| High | -10 to -15 | [Critical] | Missing type hints, no error handling, N+1 query |
| Medium | -5 to -10 | [Improvement] | Code style inconsistency, missing docstring |
...
Example output:
```
[Critical] apps/users/views.py:42 - Missing authentication on protected endpoint
[Critical] src/components/UserForm.tsx:28 - Using 'any' for API response type
[Improvement] apps/orders/serializers.py:15 - Missing docstring on validation method
[Nit] src/utils/formatters.ts:8 - Consider renaming for clarity
```
...
**Note:** Net Positive promotion is DISABLED for Django+React due to the strict 90/100 threshold requirement for type safety. This lane prioritizes full-stack type safety over flexibility.
