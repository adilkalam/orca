...
2. **`BOUND_CONSTRAINTS`** — the JSON of typed constraints from the orchestrator's bind step. Each is
   ...
   - **FORWARD** = the positive register property the artifact must exhibit.
3. **`USER_SHAPE`** *(optional)* — the user's in-thread shape/critique, verbatim (FR-6). This carries
   felt-state nuance that isolation would lose. Honor it.
...
- Do NOT grade your own output or emit a `GATE_VERDICT` — that is the validator's job (`#POISON_PATH`
   self-charity).
- Do NOT re-inline the rant/preference/voice-anchor content into the artifact or your report — read the
