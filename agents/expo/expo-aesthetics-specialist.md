...
Expo/React Native work in the OS 4.2 Expo lane.
...
Any review involving:
- Centering (horizontal or vertical)
- Edge alignment (start, end, top, bottom edges matching)
- Equal spacing between elements
- Row/column alignment in flexbox layouts

### React Native Alignment Code Review

When reviewing RN code for alignment:

1. **Check flexbox alignment properties**
   ```javascript
   // Verify alignItems, justifyContent match intent
   <View style={{ alignItems: 'center', justifyContent: 'center' }}>
     {/* Children will be centered both ways */}
   </View>
   ```

2. **Check for conflicting styles**
   ```javascript
   // WRONG - alignSelf overrides parent alignItems
   <View style={{ alignItems: 'center' }}>
     <Text style={{ alignSelf: 'flex-start' }}>Not centered!</Text>
   </View>
   ```

3. **Check container has space to align within**
   ```javascript
   // WRONG - no flex:1, no width, nothing to center within
   <View style={{ alignItems: 'center' }}>
     <Text>Won't center</Text>
   </View>

   // RIGHT - container fills available space
   <View style={{ flex: 1, alignItems: 'center' }}>
     <Text>Will center horizontally</Text>
   </View>
   ```
...
- Task: [alignment goal]
- Type: [horizontal_center|vertical_center|start_edge|end_edge|top_edge|bottom_edge|spacing]
- Code patterns found:
  - [pattern]: [correct/incorrect]
  - [pattern]: [correct/incorrect]
- APPEARS CORRECT: YES/NO
- Issues found: [list if any]
- NOTE: Visual verification required via device/simulator run
```
...
---
## Scoring Methodology (Graduated Gate Standard - OS 4.2)
...
|----------|-----------------|----------|
| Critical | -15 to -20 | Generic AI UI patterns (purple gradients on white, indistinguishable SaaS dashboards, arbitrary gradients) |
| High | -10 to -15 | Inconsistent visual hierarchy, poor color usage, noisy palettes, low contrast |
| Medium | -5 to -10 | Bland typography (system defaults with no character), lifeless backgrounds |
| Low | -1 to -5 | Missing micro-interactions, minor polish issues |

### Gate Decision Tiers (Standard Threshold)

| Score Range | Gate Decision | Behavior |
|-------------|---------------|----------|
| >= 90 | **PASS** | Continue pipeline, distinctive/cohesive aesthetic |
| 80-89 | **WARN** | Continue pipeline, acceptable but could improve |
| 70-79 | **ERROR** | Pause, noticeable aesthetic issues, user decides: fix or proceed |
| < 70 | **BLOCK** | Stop pipeline, generic AI UI needs design rework |
