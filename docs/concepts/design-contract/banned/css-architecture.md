# Banned: CSS architecture — utility sprawl is autocompleting, not designing
...
(These are from `docs/concepts/llm-css-manifesto.md`, not Adil's spoken critique catalog. They are the load-bearing framing for this pattern-family. The manifesto is the causal model; this banned file is the catalog entry.)
...
- **Refused (P0, already caught by `banned/colors.md`):** raw Tailwind *palette* utilities — `bg-blue-500`, `text-gray-700`, etc. These are design decisions invented inline.
- **Advisory (this rule):** large/repeated *utility clusters* — a `className` with 6+ layout/spacing/type tokens repeated across components. The signal that design authority has scattered.
...
False positives are acceptable here because the rule only logs. It is a *smell* detector for scattered authority, not a blocking gate. Raw palette utilities are the blocking concern and are owned by `banned/colors.md` / `tailwind-palette-utilities` (P0).
...
- **Sibling rule:** `banned/colors.md` — the palette-only stance (raw Tailwind palette utilities refused at P0). Colors is the *palette* axis; this file is the *architecture* axis. Together they cover "where does design authority live."
