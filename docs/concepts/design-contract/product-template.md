...
Stitch-spec convention. "brand" = marketing/editorial surface (landing pages, marketing sites, brand campaigns). "product" = app UI / functional surface (dashboards, tools, workflows). The register conditions which global rants apply universally vs. register-conditionally.
...
- "Boring." (unbound)
- "Generic." (unbound)
- "AI slop." (global rant — lives in `rants/`, not PRODUCT.md)
- "Tailwind defaults." (global rant — lives in `rants/colors.md`)

The anti-references in PRODUCT.md are **project-specific**. Global rants live in the global catalog. Cite by **name** — that's what makes Do's-and-Don'ts in DESIGN.md actionable ("Forbidden: Linear-style task UI density. The product is editorial, not engineering tooling.").
...
- Project-scoped refusals AND reversals (beyond global rants).
...
- Global catalog: `~/.claude/docs/concepts/design-contract/`
  - `rants/` — 10 anti-pattern files (universal refusals)
