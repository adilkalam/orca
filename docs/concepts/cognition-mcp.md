...
- `verbose: true` -- Full echo response (backward compatible). Returns the complete content, quality, status, and session context. Use for single-call commands where the echo IS the output (e.g., /think).
...
- Per-project: `{project}/.claude/.orca/cognition/sessions/{id}/*.jsonl` (when `projectPath` is provided)
