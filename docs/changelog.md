...
### Hook Infrastructure Overhaul (9 bugs fixed)
- Fixed auto-deploy.sh: moved from PreToolUse to PostToolUse (was syncing stale content)
- Fixed post-tool-use.sh: reads stdin JSON instead of nonexistent env vars
- Fixed gate-enforcement.sh: reads stdin JSON, early-exit when no phase_state.json, removed unsafe `set -u`
- Wired gate-enforcement.sh as PreToolUse in project settings
- Consolidated session-end.sh: removed duplicate Stop registration, kept SessionEnd only
- Removed duplicate SessionStart and PreCompact entries in settings.local.json
- Deleted 4 vestigial hooks: load-design-dna.sh, detect-project-type.sh, auto-activate-skills.sh, git-tracking-guard.sh

### Memory Lifecycle Upgrade
- PreCompact extraction: captures last 50 transcript entries as context snapshot before compaction
- SessionEnd summary: structured session summary with files changed, errors, and pre-compact context
- Session-start loading: injects previous session summary (if < 24h old) into startup context

### Agent Cleanup
- Removed `model: inherit` from 46 agents (Opus 4.5 is the default; specifying it is redundant)
- Fixed broken YAML in 2 Next.js agents (line-broken tool names)
- Removed non-standard tool declarations from 2 iOS agents (curl, xcrun available via Bash)
- Removed design-review-from-screenshot from dependency graph (archived command)

### /plan Command Fix
- Restructured plan.md to enforce requirements-first flow
- Requirements folder initialization now happens BEFORE any cognition analysis
- Added explicit routing guard preventing output to .claude/cognition/
- Simplified --problem-solve section from ~200 lines of templates to ~20 lines of clear intent

### Documentation Sync
- Updated counts: 117 agents, 32 commands, 13 pipelines across all reference docs
- Added MCP-to-subagent limitation documentation (MCP tools do not propagate to Task-spawned subagents)
- Verified crawl4ai tool name references (all active code uses correct names)

### SEO Pipeline (carried from v4.3)
- Full SEO content pipeline with research, brief, draft, and quality gate stages

### Cognition-MCP Substrate Schema v2 (carried from v4.3)
- 39 cognitive operations with enhanced mental model templates

### Version Bump
- Updated all version references from OS 4.2/4.3 to OS 5.0 across 130+ files
- Updated os-dependency-graph.yaml version to 5.0
- Updated all phase-config version fields to 5.0

---

## 2026-01-23

### Cognition Command Optimization

Based on LLM Cognitive Reasoning Optimization research:
- `/think --deep`: Extended thinking mode (8-12+ thoughts with review/synthesis checkpoints)
- `/contemplate`: Problem-type detection with evidence-backed routing table
- `/deepthink`: External verification questions + enhanced modes (MAP+causal, INVERT+reflexion, etc.)
- `/problem-solve`: Phase gates with verification checkpoints
- All commands: Cross-command handoff guidance in outputs

---

## v4.3.0 - 2026-01-19

### Cognition Tooling v2

**Command Separation:**
- `/deepthink` is now for **divergent exploration** - route-based modes for finding questions, hypotheses, and unknowns
- `/problem-solve` is now for **convergent decision-making** - the original 8-step ORIENT→ANTICIPATE→GENERATE→EVALUATE→COMMIT pipeline

**New /deepthink (Exploration)**
- Route-based modes: MAP, INVERT, PERSPECTIVES, EDGES, META, DEEP
- Mandatory self-check at every routing decision
- Depth gates ensure genuine insight, not shallow output
- Graduated intensity: `--light`, (default), `--full`
- Valid outputs: "I'm more confused in useful ways", questions discovered, hypotheses worth testing

**New /problem-solve (Convergent Decisions)**
- Renamed from original `/deepthink`
- 8-step pipeline unchanged: orchestration, systems, pre-mortem, tree, decide, challenge, ulysses, meta
- Variants: `--quick` (3 steps), `--risk` (4 steps), `--strategic` (5 steps), `--incident` (3 steps)

**/plan Updates:**
- `--deepthink` flag renamed to `--problem-solve`
- Creates `00-problem-solve-analysis.md` (was `00-deepthink-analysis.md`)
- All functionality unchanged

### Documentation Updates

- Updated `commands/deepthink.md` - new exploration command
- Created `commands/problem-solve.md` - convergent decision pipeline
- Updated `commands/plan.md` - `--problem-solve` flag
- Updated `docs/reference/os-dependency-graph.yaml` - new command entries
- Updated `quick-reference/ORCA-OS/ORCA-commands.md` - command table
- Updated `quick-reference/guide-think-complex.md` - automated pipelines section
- Updated `quick-reference/readme-plan.md` - flag references

### Count Updates

- Total commands: 30 (was 29)
...
Updated all version references across the system to v4.2.0.
...
- Updated quick-reference files with v4.1 versioning
...
- Check Workshop + code-index.db before ProjectContext
