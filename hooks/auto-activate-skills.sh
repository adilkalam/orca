...
if printf "%s" "$TASK_TEXT" | grep -Eiq "design|ui|ux|typography|spacing|layout|component|swiftui"; then
  echo "[$TS] Activate design helpers: (placeholder) Fluxwing/uxscii for task: ${TASK_TEXT}" >> "$LOG_DIR/skills.log" 2>/dev/null || true
  echo "Design helpers activation logged."
else
  echo "[$TS] No design activation for task: ${TASK_TEXT}" >> "$LOG_DIR/skills.log" 2>/dev/null || true
