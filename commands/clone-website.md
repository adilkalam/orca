...
**Goal:** Use SAME.dev's cloning playbook (via web analysis) to produce a high‑fidelity UI/structure spec for a target site, then hand off to OS 5.2 unified `/orca` pipeline for implementation. This command focuses on **spec and structure**, not blindly autogenerated code.
...
## 5. OS 5.2 Integration (Context & Memory)
...
- Actual implementation goes through the OS 5.2 unified `/orca` pipeline with quality gates, not as a one-off clone.
