---
description: "OS 4.1 orchestrator entrypoint for native iOS tasks"
...
# /ios - iOS Lane Orchestrator (OS 4.1)
...
```
Bash({
  command: 'TRACE_ID="ios-$(date +%Y%m%dT%H%M%S)-$(LC_ALL=C tr -dc a-z0-9 </dev/urandom | head -c 4)" && mkdir -p .claude/telemetry/sessions && echo "{\"type\":\"pipeline_start\",\"trace_id\":\"$TRACE_ID\",\"ts\":\"$(date -u +%Y-%m-%dT%H:%M:%SZ)\",\"data\":{\"domain\":\"ios\",\"task\":\"$TASK_SUMMARY\",\"mode\":\"$MODE\"}}" >> .claude/telemetry/sessions/trace-$TRACE_ID.jsonl && echo $TRACE_ID',
...
### 1.1.1 Reflexion Loading (OS 4.1)
...
Delegate to `ios-light-orchestrator` with Context Inheritance:
