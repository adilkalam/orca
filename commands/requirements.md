---
description: "Unified OS 7.1 planner -- requirements + RA blueprint (no implementation)"
...
If no flag is provided, `/requirements` will analyze the task and **recommend** a tier:
...
-> Suggest running: /requirements "Refactor CSS architecture..." --complex
...
All cognitive work in /requirements MUST use the correct tool. Do not rely on defaults.
...
**Model constraint:** If you spawn ANY Task subagent during /requirements, you MUST pass `model: "opus"`. Planning requires deep reasoning -- never downgrade to Sonnet or Haiku.
...
**Prerequisites**: The requirements folder from Section 1 AND discovery questions from Section 3 MUST be complete before this section runs. No cognition-mcp calls before AskUserQuestion.
...
The key difference: `--explore` produces a TENTATIVE brief (not committed), while standard `/requirements` produces a COMMITTED spec.
...
```bash
/requirements --from-brief                    # Auto-detect most recent brief
/requirements --from-brief <path>             # Use specific brief
```
...
Output follows standard `/requirements` completion (Section 5 below).
...
No production code should be written during `/requirements`.

---

### Design Weight Detection

Detect design relevance via keyword matching in the task description:

**Keywords**: CSS, styling, design, visual, layout, responsive, theme, tokens, colors, typography, UI, UX, component, aesthetic, refactor.*css, refactor.*style

**Scoring**:
- `high`: 3+ keyword matches
- `medium`: 1-2 keyword matches
- `low`: 0 keyword matches

Add to `metadata.json`: `"design_weight": "high|medium|low"`

---
## 5. Next Steps -- Execute with /orca

After `/requirements` completes, suggest the matching domain command with the **same tier**:
