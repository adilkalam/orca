...
ts="$(timestamp)"
if [ -z "$OUT" ]; then
  OUT=".orca/orchestration/evidence/screenshots/snap-$ts.png"
else
  case "$OUT" in
    .orca/orchestration/evidence/*) ;; # ok
    *) OUT=".orca/orchestration/evidence/screenshots/$(basename "$OUT")" ;;
  esac
fi

# Write MCP request file for a screenshot capture agent to fulfill
REQ=".orca/orchestration/evidence/requests/${ts}-screenshot.json"
