...
Use `interfaces-that-feel` peer skill's persona table (joy/efficiency/trust/urgency). Auto-select 2-3 personas relevant to the product register. For each, walk through the primary user action and list specific red flags.
...
**Deterministic detector location:** `mcp/design-detector/` in ORCA-OS. Rules read from `~/.claude/docs/concepts/design-contract/detector-rules.json` (hybrid: JSON patterns + named TS handlers). Runs automatically as part of critique.
