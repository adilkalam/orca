...
## Directional-not-perspective (NEW — same as /animate)

2D only. No perspective. See `~/.claude/docs/concepts/design-contract/preferences/motion-references.md`.

## Radiant as ambient-background option (NEW)

Before building a custom shader / Three.js scene for ambient background, consider `~/.claude/docs/concepts/design-contract/radiant-shaders/` — 88 drop-in canvas/WebGL shaders. Embed via iframe, control via postMessage, apply color filter for palette match.
