...
## Three-tier routing (NEW — runs FIRST)

Use `motion-design-principles` peer skill for routing:
...
Present the tier decision to the user. NEVER auto-escalate. See `skills/motion-design-principles/SKILL.md`.

## Directional, not perspective (NEW)

2D translation, fades, clip-path reveals, mask animations allowed. 3D transforms, tilt effects, parallax-in-z-axis, device-orientation effects REFUSED. See `~/.claude/docs/concepts/design-contract/preferences/motion-references.md`.

## Craft source (NEW)

All motion craft — custom easing curves, durations, press states — from `skills/animation-engineering/SKILL.md` (Emil Kowalski's skill).
