...
If the user responds, append to the current project's `.orca/design-rants-pending.md` in this format:
...
Create `.orca/` in the current project if absent. Do NOT write to `~/.claude/` or to the ORCA-OS source tree directly. Pending entries are swept later via `/impeccable extract rants`.
