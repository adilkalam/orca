...
| `--analogy` | Analogical reasoning -- source domain to target mappings |
...
| `--tree` | Tree of thought | Branch exploration with pruning |
| `--beam` | Beam search | Tracking top-N candidates across iterations |
| `--mcts` | Monte Carlo Tree Search | Visit/value-weighted exploration |
| `--argue` | Structured argumentation | Claim, premises, evidence, counter-arguments |
| `--analogy` | Analogical reasoning | Mapping a source domain onto a target |
| `--stats` | Statistical reasoning | Data-grounded claims with confidence and caveats |
| `--simulate` | Scenario simulation | Stepping through initial conditions to final state |
| `--optimize` | Optimization | Objective, constraints, variables, tradeoffs |
...
**Persisted artifacts**:
- `.orca/cognition/YYYYMMDD-HHMM-slug.md` - Summary files (created by command specs, not the MCP)
- Per-project: `{project}/.claude/.orca/cognition/sessions/{id}/*.jsonl` - Session logs (when `projectPath` provided)
...
- `commands/problem-solve-local.md` - Full 8-step pipeline
