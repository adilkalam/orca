...
- `design-detector` (at `mcp/design-detector/`) is **NOT an MCP server** — it is a CLI tool (`node mcp/design-detector/bin/designcheck.js`, NOT npx). It lives in `mcp/` pragmatically alongside other support tooling. Fork of Bakaus's Impeccable detector. Hybrid JSON-rules + named-handler architecture. Rules read from `docs/concepts/design-contract/detector-rules.json` (override via `DESIGN_COLLECTION_PATH`) and applied live via `runExternalRules` in `src/detect-antipatterns.mjs`. **11 pattern-based rules live** (8 P0: tailwind-palette-utilities, tailwind-hex-values, reflex-fonts, geist-imports, purple-pink-gradients, gradient-text, side-stripe-borders, inset-highlight-shadow; 2 P1: default-ease-transition, bouncy-easing; 1 advisory-marked P1: utility-sprawl). 3 handler-based rules deferred as stubs (chamfer-stack, asymmetric-heading-margins-missing, missing-typography-junctions); 3 removed from scope (ios-calculator-default, pretend-variation, pixel-misalignment). **Severity-aware exits (2026-07-31):** EXIT 2 only when a finding's effective severity is P0/P1; advisory-only runs print findings JSON but EXIT 0. Self-suppresses owner overrides from `{project}/.design-overrides.json` (`DESIGN_OVERRIDES_PATH`). See `mcp/design-detector/README.md`.
...
- `swift-design-detector` (at `mcp/swift-design-detector/`) is **NOT an MCP server** — it is a Bash-invoked local CLI (`swiftdesigncheck detect --json <path>`), the iOS analogue of `design-detector`. **Swift** (SwiftPM executable; built against Swift 6.3.2, `swift-syntax` pinned to the 603.x tag family), not TypeScript/Python. It statically scans a Swift/SwiftUI file via an in-process **SwiftSyntax AST walk** (NOT regex). Same capture contract as the web detector, now **severity-aware (2026-07-31)**: `EXIT 0 + "[]"` (clean, STDOUT) / `EXIT 0 + findings JSON` (advisory-only — report, never blocks) / `EXIT 2 + findings JSON` (blocking — at least one P0/P1 EFFECTIVE-severity finding; per-project severity override, else corpus default; STDERR, `2>&1`-capturable) / `EXIT 1` (usage — including plural paths: **v1 takes ONE file per invocation**, the validators loop the files). Reads the Swift rule file from `docs/concepts/ios-design-contract/detector-rules.swift.json` (override via `SWIFT_DESIGN_RULES`); per-project token-dir scoping comes from `.design-detector.swift.json` (override via `SWIFT_DESIGN_CONFIG`, schema in `docs/concepts/ios-design-contract/detector-config-schema.md`); owner-override self-suppression from `{project}/.design-overrides.json` (`SWIFT_DESIGN_OVERRIDES`). 13 rules: 8 P0 (off-palette-hue, raw-hex-outside-tokens, hue-coded-category, tailwind-palette-hex, gradient-fill, display-font-below-floor, ios-default-reflex, unjoined-unit-baseline) + 5 P1 (system-font-reflex, magic-number-spacing, shadow-reflex, spring-overshoot, mono-fatigue). The keystone correctness point is **rule-class-aware token-dir scoping** (`scope_in_token_dirs`): off-palette/hue-coded/tailwind/gradient slop STILL FIRES inside token dirs (the slop is defined there) while raw-hex/system-font/magic-spacing suppress. The `bin/swiftdesigncheck` wrapper resolves the prebuilt release binary → `swift run` fallback → graceful non-crashing skip when no Swift toolchain is present. Consumed-by: the **ios-design** overlay (`/ios-impeccable` T0 inline report + `ios-design-validator` on the gated tiers + the hook design floor). See `mcp/swift-design-detector/README.md`.
...
Google PageSpeed Insights + Chrome UX Report (CrUX) analysis. 16 tools covering Lighthouse metrics, Core Web Vitals field data, page comparison, and optimization recommendations. External fork of [ruslanlap/pagespeed-insights-mcp](https://github.com/ruslanlap/pagespeed-insights-mcp) built into ORCA (source vendored at `mcp/pagespeed-insights-mcp/`, built with `npm install && npm run build`, deployed like project-context-server).

```json
{
  "pagespeed-insights": {
    "command": "node",
    "args": ["/Users/adilkalam/.claude/mcp/pagespeed-insights-mcp/dist/index.js"],
    "env": {
      "GOOGLE_API_KEY": "<PageSpeed Insights API key>"
    }
  }
}
```

**Key tools:** `analyze_page_speed` (full Lighthouse run: performance/accessibility/best-practices/seo/pwa), `get_performance_summary`, `crux_summary` (real-world Core Web Vitals), `compare_pages` (side-by-side URL comparison), plus recommendation and batch helpers (16 total).

**Auth:** Google API key with PageSpeed Insights API enabled, passed via `GOOGLE_API_KEY` env var (validated at startup by zod).
**Source:** `mcp/pagespeed-insights-mcp/` -> deployed to `~/.claude/mcp/pagespeed-insights-mcp/`
**Projects:** peptidefox, obsidian-peptides, rvry (project-scoped via .mcp.json)
**Scope:** Project-scoped only (configured in each project's `.mcp.json`, NOT global)
