...
**Last Updated:** 2026-07-31 (design-lane three-tier restructure)
**Version:** OS 7.1
...
Design-fork commands. **All nine load the `impeccable-hub` skill** as their aesthetic baseline (the felt-state spine + voice anchors + banned rules + preferences + detector contract — it POINTS to `design-contract/` refs, no inlined copies), read `{project}/.claude/PRODUCT.md` (strategic) and `{project}/.claude/DESIGN.md` (visual contract — Stitch spec), and reference the owner-gated aesthetic-capture protocol (no closing capture question). Optional parallel hygiene check via `bash {project}/scripts/audit-design.sh` informs but does not gate — the user's eye is the taste ceiling, not the script.
...
> **2026-06-03 design-system totality rethink (Phase 2 — all 8 design commands converted).** The eight design commands were converted to load the `impeccable-hub` skill (was `interfaces-that-feel`) and read `PRODUCT.md`/`DESIGN.md` (was the deprecated `aesthetic.md`). **Build-producing commands route through the one shared design lane** (`docs/reference/design-lane.md`): the command (main-thread orchestrator) binds typed FORBIDDEN/FORWARD constraints via a cognition `checkpoint`, spawns a **separate** `design-builder`, then a **separate fresh-context** `design-validator` (returns `GATE_VERDICT: PASS\|BLOCK` via the LOCAL detector `node mcp/design-detector/bin/designcheck.js` — NOT `npx`), and branches (N=2 → escalate). The model never grades its own output; the old self-graded `ui-quality-audit` handback gate is replaced by the separate validator. **Build-producers (route the lane):** `/refine`, `/simplify`, `/fortify`, `/recraft` (Routes A/B only), `/motion-design`. **Diagnostic / contract commands (do NOT route the lane):** `/document` (generates DESIGN.md), `/design-audit` + `/design-critique` (produce reports), `/recraft` Route C (contract edit, no UI artifact). Honest ceiling: hard-on-named-slop, advisory-on-taste — the user's eye is the taste ceiling. **Spec:** `.orca/requirements/2026-06-03-2251-design-system-totality-rethink/`. *(Superseded in part by the 2026-07-31 three-tier restructure below — the single always-full lane shape is history.)*

> **2026-07-31 three-tier restructure (canon `docs/reference/design-lane.md`).** Both lane commands (`/impeccable` web, `/ios-impeccable` iOS) now route a request to one of three tiers, **biased toward fast**: **T0 `--tweak`** — in-thread edit + inline detector REPORT, ZERO agent spawns, no phase_state, no gate (the owner's live eye is the verifier; the banned-core is inlined in the command body). **T1 direct (THE DEFAULT** for a well-defined single-verb request) — the main agent builds in-thread under bound constraints (owner brief stays verbatim by construction), then exactly ONE fresh-context judge (`design-validator` / `ios-design-validator`); PASS writes the canonical gate (`gates.design_lane` / `gates.ios_design_lane`) **via the Write tool, NEVER Bash**, and the hook floor re-runs the detector. **T2 `--full`** (freeform multi-verb / thin brief) — architect plan, then per task bind → builder (**OWNER_REQUEST_VERBATIM**) → validator → branch (N=2 → escalate with the diff + `git stash create` revert handle; remaining tasks HALT on escalation; `escalated: true` is the sanctioned exit). Owner overrides write back to `{project}/.design-overrides.json` **immediately at bind**. Aesthetic capture is **owner-gated** (no per-turn question). Both detectors have **severity-aware exits** (EXIT 2 only on blocking P0/P1 effective severity). The hook design floor + detector path are **harness/E2E-proven (2026-07-31)**.
...
### `/impeccable` - Design lane, three tiers (bias-to-fast)
```bash
/impeccable --tweak "nudge the card padding to 20px"   # T0: in-thread, zero spawns, report-only
/impeccable --typeset app/pricing/page.tsx             # T1 (THE DEFAULT): in-thread build + one judge
/impeccable make this header breathe                   # freeform single-verb → T1
/impeccable --full "clean up the pricing page"         # T2: architect → builder → validator per task
/impeccable --craft "a settings panel"                 # shape → comp pick → T2
/impeccable --teach                                    # write PRODUCT.md (setup, no lane)
/impeccable --extract aesthetic                        # sweep the pending ledger into banned/
```
Routes a request to a tier per the canon (`docs/reference/design-lane.md`) and executes it. Explicit flags are deterministic (`--tweak` → T0, `--<verb>` → T1, `--full` → T2); freeform classifies in one line, then goes — **bias-to-fast, when in doubt route DOWN a tier**. **T0** edits in-thread with the detector as an inline REPORT (never blocks); the T0 banned-core is deliberately **inlined in the command body** (the only lane content it carries — T1/T2 knowledge stays in the hub + `design-contract/`, `#POISON_PATH`). **T1 (THE DEFAULT)** binds constraints via a cognition checkpoint, builds in-thread, spawns exactly ONE fresh-context `design-validator`, and writes `gates.design_lane` via the Write tool on PASS (hook floor applies; BLOCK → fix in-thread, MAX N=2, then escalate). **T2** spawns `design-architect` to plan ordered verb-tasks, then per task: bind → `design-builder` (owner request VERBATIM) → `design-validator` → branch; stash-snapshot revert handle; halt-on-escalation. `--teach`/`--document`/`--extract` are setup flags (no lane). **Owner-override precedence (2026-06-23):** a typed `OVERRIDE` (`{suppresses, scope, value, provenance}`) is bound ONLY from an explicit owner instruction and **written back to `{project}/.design-overrides.json` immediately at bind**, so the suppressed rule stops firing for its `scope` on future runs (`docs/reference/design-lane.md` §Precedence). The owner outranks the aesthetic outranks the detector.

> **CSS Architecture / doctrine B (2026-05-29).** The `/impeccable` spine carries a **CSS Architecture** subsection under Frontend Aesthetics Guidelines (peer to Typography / Color & Theme / Layout & Space): design authority lives in a named role/token vocabulary; agents compose from it, taxonomy-first; cascade/`@layer` greenfield default; brownfield respects the detected CSS approach; no raw palette utilities; **no absolute Tailwind ban** (utility SPRAWL is advisory only via the `utility-sprawl` detector rule — logs, never blocks). Refusal: `banned/css-architecture.md`. Positive move: `preferences/css-architecture.md`. Seam discipline: the spine states the PRINCIPLE; enforcement mechanics live in the detector floor + lane builders. Source: `docs/concepts/llm-css-manifesto.md`.

### `/ios-impeccable` - iOS/SwiftUI design lane, three tiers (ADDITIVE overlay)
```bash
/ios-impeccable --tweak "bump the dose label to titleScale"   # T0: in-thread, zero spawns, report-only
/ios-impeccable --colorize DosingCardView.swift               # T1 (THE DEFAULT): in-thread build + one judge
/ios-impeccable --full "clean up the dosing flow"             # T2: architect → builder → validator per task
```
The SwiftUI sibling of `/impeccable` — same three tiers, same canon (`docs/reference/design-lane.md`, REUSED not duplicated), with the iOS deltas: the `ios-impeccable-hub` skill (the iOS standing aesthetic), the **Swift** detector `swiftdesigncheck` (NOT `designcheck.js`; **invoked ONCE PER artifact file** — plural paths in one invocation = EXIT 1 DETECTOR-ERROR), and the `gates.ios_design_lane` gate. **T0 `--tweak`**: in-thread edit + inline `swiftdesigncheck` report (banned-core inlined in the command body). **T1 (THE DEFAULT)**: in-thread build + exactly ONE fresh-context `ios-design-validator` judge + Write-tool gate write with the hook floor. **T2 `--full`**: `ios-design-architect` plan, then per task bind → `ios-design-builder` (owner request VERBATIM; rejects web reflexes) → `ios-design-validator` → branch (N=2 → escalate, stash revert handle, halt-on-escalation). **Runs ADDITIVELY alongside the `/ios` correctness gates — it does NOT replace `ios-standards-enforcer`, `ios-ui-reviewer`, or `ios-verification`;** the validator fills the former `design-dna-guardian` role for iOS. Verb subset v1 (12): layout, typeset, colorize, bolder, quieter, delight, harden, polish, distill, adapt, clarify, animate (excludes overdrive/threejs — no clean SwiftUI analogue — and optimize, exclusion now explicit). **Owner-override precedence (2026-06-23):** a typed `OVERRIDE` bound ONLY from an explicit owner instruction, **written back to `{project}/.design-overrides.json` (`SWIFT_DESIGN_OVERRIDES`) immediately at bind**. Severity is per-project; `ios-default-reflex` + `system-font-reflex` are **P0 owner-instructed**. **Status: active — the hook design floor + detector path are harness/E2E-proven (2026-07-31).** **Spec:** `peptidefox-ios/.orca/requirements/2026-06-17-2153-ios-impeccable-adaptation/`.

### `/motion-design` - Heavy motion/animation orchestrator
```bash
/motion-design "fade the hero in on scroll"
```
Loads the `impeccable-hub` skill (the aesthetic / felt-state baseline) + `animation-engineering` + `motion-design-principles` (+ `lenis-integration` when scroll-related). Presents 3-tier routing (CSS default; GSAP/Three.js require user confirmation). Refuses 3D transforms, tilt, parallax-Z. For a light pass, use `/refine --animate`. **Routes the shared design lane (build-producer):** the entry bind emits motion FORBIDDENs (`bouncy-easing`/`default-ease-transition`/`layout-transition` + 3D/tilt/parallax-Z banned refusals) + FORWARD felt-state, recorded to `planning.bound_constraint_ids`; `design-builder` produces the motion under those ids; the fresh-context `design-validator` judges motion-feel (the detector is motion-blind to timing/emotion) and returns `GATE_VERDICT: PASS\|BLOCK`, looping N=2 then escalating. Skipping the bind ⇒ no bound ids ⇒ `BLOCK: no bound constraints`.
