...
## Design Lane (3 Agents)

Design lane, three tiers biased toward fast (2026-07-31 restructure; canon `docs/reference/design-lane.md`, referenced never copy-pasted). **T0 `--tweak`** spawns NOTHING (in-thread edit + inline detector report). **T1 direct (THE DEFAULT)**: the main agent builds in-thread, then spawns exactly ONE fresh-context judge (`design-validator`) — no builder spawn. **T2 `--full`**: `design-architect` plans, then per task `design-builder` → `design-validator` (N=2 → escalate). Hub skill: `skills/impeccable-hub/SKILL.md` (the standing aesthetic; points to `design-contract/` refs, no inlining).

| Agent | Purpose |
|-------|---------|
| `design-architect` | **T2-only planner.** Decomposes a freeform/multi-verb request into ordered verb-tasks (layout/typeset/colorize/harden/…), each with forbidden/forward/override constraint seeds + target files. Class-scopes the sweep. **Emits a typed `OVERRIDE` constraint** (`OWNER-OVERRIDE\|suppresses=…\|scope=…\|value=…\|provenance=…`) ONLY from an explicit owner instruction that contradicts a standing rule — the channel that makes the owner outrank the aesthetic/detector (`docs/reference/design-lane.md` §Precedence; schema `docs/concepts/design-overrides-schema.md`). Plans only — never implements, never spawns. T1 does not use it. |
| `design-builder` | **T2-only producer.** Receives the owner's request **VERBATIM** (`OWNER_REQUEST_VERBATIM` — summaries and re-encodings are abolished); builds the front-end artifact under the bound constraint ids with the hub injected. Does NOT self-grade. T1 has no builder spawn — the main agent builds in-thread. |
| `design-validator` | Fresh-context judge on **BOTH gated tiers** — on T1 it is the **single judge** of the main agent's in-thread build; on T2 it judges each `design-builder` task. Runs the LOCAL detector (`node mcp/design-detector/bin/designcheck.js`; severity-aware exits), judges bound ids, returns `GATE_VERDICT: PASS\|BLOCK`. **Subtracts owner-sanctioned findings before the verdict:** for each active `OVERRIDE`, a detector finding whose `ruleId` + path falls under the override's `suppresses` + `scope` is removed from `UNSATISFIED_CONSTRAINTS` and downgraded to an `owner-sanctioned` advisory — an owner-sanctioned P0 no longer forces BLOCK (the owner outranks the floor). Hard-on-named-slop, advisory-on-taste. Never sees the build reasoning. |

> **Status (2026-07-31):** active — all 9 design commands aligned to the tier canon; the hook design floor + detector path are harness-proven.

---

## iOS Design Lane Overlay (3 Agents)

Additive iOS/SwiftUI design overlay (2026-06-18 ios-impeccable-adaptation; three tiers since 2026-07-31). The SwiftUI sibling of the Design Lane — same tier model (canon `docs/reference/design-lane.md`, REUSED not duplicated): T0 `--tweak` spawns nothing; **T1 direct (THE DEFAULT)** spawns only the fresh-context `ios-design-validator` judge; **T2 `--full`** runs `ios-design-architect` → `ios-design-builder` → `ios-design-validator` per task (N=2 → escalate). The validator runs the **Swift** detector (`swiftdesigncheck`, NOT `designcheck.js`), **once per artifact file**. Hub skill: `skills/ios-impeccable-hub/SKILL.md` (the iOS standing aesthetic: color principle + SwiftUI banned rules + Swift detector contract). **This overlay composes ADDITIVELY with the `/ios` correctness lane — it does NOT replace `ios-standards-enforcer`, `ios-ui-reviewer`, or `ios-verification`.** Verb subset v1: layout, typeset, colorize, bolder, quieter, delight, harden, polish, distill, adapt, clarify, animate (excludes overdrive/threejs — no clean SwiftUI analogue — and optimize).

| Agent | Purpose |
|-------|---------|
| `ios-design-architect` | **T2-only planner.** Decomposes a SwiftUI request into ordered verb-tasks, each with forbidden (Swift detector rule ids) / forward (felt-state) / override seeds + target `.swift` files. Class-scopes the sweep. **Emits a typed `OVERRIDE` constraint** ONLY from an explicit owner instruction that contradicts a standing rule (e.g. the owner sanctioning soft-red against the palette P0) — the channel that makes the owner outrank the aesthetic/detector (`docs/reference/design-lane.md` §Precedence; schema `docs/concepts/design-overrides-schema.md`). Plans only — never implements, never spawns. T1 does not use it. |
| `ios-design-builder` | **T2-only SwiftUI producer.** Receives the owner's request **VERBATIM** (`OWNER_REQUEST_VERBATIM`); builds under the bound constraint ids; loads `ios-impeccable-hub` + reuses `ios-swiftui-specialist` knowledge; rejects web reflexes (OKLCH/`gap`/CSS/`px`) via a `#POISON_PATH` guard; self-checks with `swiftdesigncheck` once per file. Does NOT self-grade. T1 has no builder spawn — the main agent builds in-thread. |
| `ios-design-validator` | Fresh-context judge on **BOTH gated tiers** — the **single judge** on T1, the per-task judge on T2. Runs the LOCAL Swift detector (`mcp/swift-design-detector/bin/swiftdesigncheck`, overridable via `SWIFT_DESIGN_DETECTOR_BIN`; severity-aware exits) **ONCE PER artifact file** (plural paths in one invocation = EXIT 1 DETECTOR-ERROR), aggregates findings, judges bound ids, returns `GATE_VERDICT: PASS\|BLOCK` + `SCORE` + `UNSATISFIED_CONSTRAINTS` + `FINDINGS`. **Subtracts owner-sanctioned findings before the verdict:** for each active `OVERRIDE`, a finding whose `ruleId` + path falls under the override's `suppresses` + `scope` is removed from `UNSATISFIED_CONSTRAINTS` and downgraded to an `owner-sanctioned` advisory — an owner-sanctioned P0 no longer forces BLOCK. Hard-on-named-slop, advisory-on-taste. **HARD-BLOCKS if the detector errors or is unavailable** (`DETECTOR-ERROR` / `DETECTOR-UNAVAILABLE`) — never degrades to a read-the-file pass. **Fills the former `design-dna-guardian` role for iOS.** Never sees the build reasoning. |

> **Status (2026-07-31):** active — the hook design floor + detector path are harness/E2E-proven. The 5 target-aware design-fork commands (`/refine` `/fortify` `/simplify` `/design-audit` `/design-critique`) detect `.swift` targets per `docs/concepts/ios-design-contract/target-routing.md`.
